function [bb,dev,stats] = eglm_glmfit(x,y,distr,varargin)
%
% This is a copy of the MATLAB GLMFIT function, but modified so that its
% super inefficient way of checking its inputs is no longer so slow.
% (Seriously, for my recent model fits it was spending almost 2x more time
% checking whether the first input was a string, than doing ALL the rest of
% the function including fitting the actual model...)
% ESBM 2022-01-04
% 
...
if nargin < 4
    ...
    if ischar(arg) % either a link name (old syntax), or a parameter name
        ...
        % ESBM edit: NEW, hopefully more efficient code.
        newSyntax = ~ismember(arg,{'identity' 'log' 'logit' 'probit' 'comploglog' 'reciprocal' 'logloglink'});
        
        % ESBM edit: OLD, super inefficient code
%         try
%             validatestring(arg, ...
...
% ESBM edit so can find the necessary function
%[anybad,wasnan,y,x,offset,pwts,N] = statremovenan(y,x,offset,pwts,N);
[anybad,wasnan,y,x,offset,pwts,N] = internal.stats.removenan(y,x,offset,pwts,N);
...
if nargout > 2
    ...
    % ESBM edit to use findable function
...
%     stats.resida = statinsertnan(wasnan, anscresid);
    stats.resid  = internal.stats.insertnan(wasnan, resid);
    stats.residp = internal.stats.insertnan(wasnan, (y - mu) ./ (sqrtvarFun(mu) + (y==mu)));
    stats.residd = internal.stats.insertnan(wasnan, sign(y - mu) .* sqrt(max(0,di)));
    stats.resida = internal.stats.insertnan(wasnan, anscresid);
    
    stats.wts = internal.stats.insertnan(wasnan,1./sqrtirls.^2);
end
