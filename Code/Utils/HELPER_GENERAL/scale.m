...
if ~isempty(varargin{v})
    if ischar(varargin{v})
        ...
        elseif numel(varargin{v})==2
            ...
            scalerefpoint = [0 1];
            v = v - 1;
            %error('expected scalerefpoint to have 1 or 2 elements');
        end;
    %NOTE: DISABLED 2009-08-02
    %elseif isnumeric(varargin{v})
    %    assert(numel(varargin{v})==2,'expected numeric input to scalerefpoint to be a vector with two elements');
    %    scalerefpoint = sign(varargin{v});
    else
        % default! scalerefpoint = 'cm'
        scalerefpoint = [0 1];
        v = v - 1;
        %error('expected scalerefpoint to be char or numeric');
    end;
end;
    ...
    if ischar(varargin{v})
        ...
        switch scaleaspect
            ...
            otherwise
                ...
                scaleaspect = 'only';
                v = v - 1;
                %error('unknown value of scaleaspect');
        end;
    else
        % default! scaleaspect = 'only'
        scaleaspect = 'only';
    ...
    if ~isempty(varargin{v})
        ...
        switch scalemode
            case {'pos','out'}
                v = v + 1;
            otherwise
                %disp(['default: setting scalemode to ''pos''']);
                scalemode = 'pos';
        end;
    end;
end;
...
% disp(['"scale"']);
% disp([' mode           : ' mode]);
% disp([' fig_handle     : ' num2str(torowvector(fig_handle))]);
% disp([' ax_handles     : ' num2str(torowvector(ax_handles))]);
% disp([' scalerefpoint  : ' num2str(torowvector(scalerefpoint))]);
% disp([' scaleaspect    : ' scaleaspect]);
% disp([' scalemode      : ' scalemode]);
% disp([' scaleconstraint: ']);
% disp(scaleconstraint);
...
%             case 'tight'
%                 % we can't simply rescale 'tightinset'. Instead, we 
%                 %  will calculate how much we have to change the 'position'
%                 %  property to get the desired change in 'tightinset'
%                 
%                 % for each axis there are three parts that add to its
%                 %  xlen (width) and ylen (height):
%                 % xlen = position(3) + (yticklabel_width+ylabel_width+...) + yticklength
%                 % ylen = position(4) + (xticklabel_width+xlabel_width+...) + xticklength
%                 
%                 % the second term, which we will call
%                 % 'xotherstuff'/'yotherstuff', is roughly constant
%                 %  regardless of how we resize the graph
%                 %  (we will neglect effects on tightinset caused by e.g. 
%                 %   the title/xlabel/ylabel extending beyond the xaxis/xaxis/yaxis)
%                 % but the third term depends in a complicated way on the
%                 %  graph size.
%                 
%                 % tick handling will fail if we are working in normalized
%                 %  units, b/c tick length depends on the *actual*
%                 %  width/height (e.g. if double width but leave height the
%                 %  same, the tick length on the screen may change, but
%                 %  the 'normalized' width/height of the figure will be
%                 %  unchanged)
%                 for i = 1:numel(ax_handles)
%                     assert(~isequal(lower(get(ax_handles(i),'units')),'normalized'),'scalemode ''tight'' requires non-normalized coordinates to correctly handle tick lengths')
%                     % probably it is possible to do these calcuations with
%                     % ActivePositionProperty = OuterPosition, but it would
%                     % be hard. When remove the ticks, the Position would
%                     % suddenly change!
%                     set(ax_handles(i),'ActivePositionProperty','Position');
%                 end;
% 
%                 
%                 % how to handle the x/y tick marks, which annoyingly will
%                 %  resize along with the figure, messing up its specified 
%                 %  height/width
%                 old_ticklen = nans(numel(ax_handles),2);
%                 old_tight_xy = nans(numel(ax_handles),2); %TightInset width/height
%                 old_pos_xy = nans(numel(ax_handles),2); %Position width/height
%                 old_tick_length = nans(numel(ax_handles),1);
%                 for i = 1:numel(ax_handles)
%                     old_ticklen(i,:) = get(ax_handles(i),'TickLen');
%                     old_tight_xy(i,:) = take(get(ax_handles(i),'tight'),3:4);
%                     old_pos_xy(i,:) = take(get(ax_handles(i),'pos'),3:4);
%                     
%                     % according to MATLAB, tick length in the 'TickLen'
%                     %  property is specified as a fraction of the maximum
%                     %  width/height of the axes.
%                     % But this calculation is not always right??
%                     old_tick_length(i) = old_ticklen(i,1).*max(old_pos_xy(i,:));
%                 end;
%                 
%                 tick_mode = 'fixed';
%                 if numel(scaleconstraints)>=4
%                     tick_mode = scaleconstraints{4};
%                 end;
%                 new_tick_length = nans(numel(ax_handles),1);
%                 if ischar(tick_mode) && isequal(tick_mode,'fixed')
%                     % keep ticks at their original size
%                     new_tick_length = old_tick_length;
%                 elseif isscalar(tick_mode)
%                     % keep ticks at a specified size
%                     new_tick_length(:) = tick_mode;
%                     tick_mode = 'fixed';
%                 elseif isnumeric(tick_mode) && numel(tick_mode)==numel(ax_handles)
%                     % keep ticks at a specified size 
%                     % (specified individually for each axes)
%                     new_tick_length = tick_mode;
%                     tick_mode = 'fixed';
%                 elseif ischar(tick_mode) && isequal(tick_mode,'auto')
%                     % pass, setting is ok
%                 else
%                     error('unknown tick mode');
%                 end;
%                 
%                 switch tick_mode
%                     case 'fixed'
%                         for i = 1:numel(ax_handles)
%                             cur_old_lrbt = lbwh_to_lrbt(get(ax_handles(i),'tight'));
%                             set(ax_handles(i),'TickLen',[0 old_ticklen(i,2)]);
%                             cur_old_lrbt_notick = lbwh_to_lrbt(get(ax_handles(i),'tight'));
%                             
%                             % deduce size of 'otherstuff'
%                             otherstuff_lrbt = 
% 
%                         end;
%                         
