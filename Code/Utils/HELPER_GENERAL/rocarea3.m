...
if nargout > 1
    ...
    switch pval_method
        case {'approximate','exact','default'}
            ...
            for c = 1:nc
                % modified 2019-03-17 to use faster method of calculating
                % rank-sum test (most of the time of the default function,
                % when called repeatedly on small-ish datasets like this,
                % is actually spent on parameter checking/etc)
                %pval(c) = ranksum(N(:,c),S(:,c),'method',pval_method);
                pval(c) = ranksum_fast(N(:,c),S(:,c),'both',pval_method);
            end;
