...
% %  Aug 2003 - JP: adapted for PVM EPI
% %
FCTNAME = 'dorkTrace';
...
if f_PVM
    ...
    TR   = acqp.PVM_RepetitionTime*1e-3;
else
    te   = acqp.EPI_TE_eff;
    ...
    TR   = acqp.IMND_rep_time;
end
if (te <= 0)
   goo = input('echo time TE/ms [50] ');
   if isempty(goo) te = 50; else  te = goo(1); end
end
te = te*0.001;
sw = acqp.SW_h;
...
for islice=ns1:ns2
expLabel = sprintf('<%s> exp#%d  sl#%d  TR=%.3f s', dir, filenum, islice, TR);
for iseg=1:nseg
