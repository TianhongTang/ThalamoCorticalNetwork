...
global STDPATH
STDPATH.pv = 'M:/mpidata/'     % test data set for DORK development
%dorkTrace('dork_N02.gf1', 11, opt('NR',[4 0],'DETREND',0) );   %10,11,12,39

PVdorkRun('dork_N02.gf1', 11, opt('NR',[4 0],'DETREND',0) );   %10,11,12,39

%--------------------------------------------------------------
function PVdorkRun( dir, filenum, optin )
%%%function PVdork( fpath, grfx_on, partialdork_on )
% %
% % Correction of Dynamic Off-Resonance Effects in K-space (DORK)
% %      J. Pfeuffer, P.-F. van de Moortele, K. Ugurbil, X. Hu, G. H. Glover, Magn.Res.Med 2001
% % ---------
...
% %  Tested for: EPI
% %
% %  Oct 2002 - rewritten for BRUKER EPI correction
% %
FCTNAME = 'PVdork';
...
fpath = sprintf('%s%s/%s', STDPATH.pv,num2str(dir),num2str(filenum));
...
dorkPath = [fpath '/fid.dork'];
%cmd = sprintf( '/bin/cp -f %s %s', fidPath, dorkPath );
%if (0 ~= unix( cmd )) error( ['Unix cmd failed:', cmd]); end
%cmd = sprintf( '/bin/chmod +w %s', dorkPath );
%if (0 ~= unix( cmd )) error( ['Unix cmd failed:', cmd]); end

fp = fopen( dorkPath, 'r+', 'ieee-be');
if (-1 == fp) error( ['Open failed : ', dorkPath]); end
