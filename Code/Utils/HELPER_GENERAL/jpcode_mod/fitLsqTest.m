...
if f_test == 4
    fun = 'funGauss';
	Pstart = [1,1,1];
	xdata=[   -6.0000   -5.2500   -4.5000   -3.7500   -3.0000   -2.2500   -1.5000   -0.7500 0    0.7500    1.5000    2.2500    3.0000    3.7500];
	ydata=[    0.100    -0.1000    0.0000    0.0009    0.0111    0.0796    0.3247    0.75  1.1 0.74   0.3247    0.0796    -0.0111    0.0009];
	plot(xdata,ydata,'.')
	
	%results = fitLsq(fun,Pstart,xdata,ydata,opt('DISPLAY','iter','MAXITER',10))
    results = fitGauss(xdata,ydata,opt('PSTART',Pstart,'NFIT',3,'verbose',2))
end
