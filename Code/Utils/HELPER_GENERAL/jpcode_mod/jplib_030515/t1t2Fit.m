...
datFit = zeros(s_dat(1), s_dat(2), s_dat(4), 8);    %% x,y,slices,[tau, 1/tau, tauErr, resnorm, M0, M0err, A, Aerr]
...
%%%         RESULTS = fitLsq('funExpMono',P,xdat,idatVoxel,opt('DISPLAY','off','MAXITER',50,'MAXFUNEVALS',50,'PLOT',f_plot));
        ...
        if RESULTS.exitflag >= 0
            datFit(ix,iy,isl,1) = Pfit(2);
            datFit(ix,iy,isl,2) = 1/Pfit(2);
            datFit(ix,iy,isl,3) = PfitErr(2);
            datFit(ix,iy,isl,4) = RESULTS.resnorm;
            datFit(ix,iy,isl,5) = Pfit(1);
            datFit(ix,iy,isl,6) = PfitErr(1);
            datFit(ix,iy,isl,7) = Pfit(3);
            datFit(ix,iy,isl,8) = PfitErr(3);
            if f_verbose
