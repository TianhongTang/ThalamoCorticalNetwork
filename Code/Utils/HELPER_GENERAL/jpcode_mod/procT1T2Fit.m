...
dopt.FITTHRES = 5;    %% SD of noise
dopt.NOISEIND = [1 1 10 10];
dopt.PLOT = 0;
dopt.AVG  = 0;
dopt.CROP = 0;
dopt.VERBOSE = 1;
...
for itype = 1:length(fittype)
    ...
    if f_write < 2
        ...
        [datFit, xdat] = t1t2FitSingleType( type , opt('PLOT',dopt.PLOT,'AVG',dopt.AVG,'CROP',dopt.CROP));
...
noiseInd = dopt.NOISEIND;
f_plot = dopt.PLOT;
f_avg = dopt.AVG;
f_crop = dopt.CROP(1) > 0;
cropI = [dopt.CROP(1:end) 0 0 0];  % 
cropI = cropI(1:4);  
f_verbose = dopt.VERBOSE;
if f_verbose == 0
    f_verbose = dopt.PLOT;
...
elseif strcmp(type, 't2s')
    refImage = 1;    %% 1: first image in series (T2 decay)
elseif strcmp(type, 't2epi')
    refImage = 1;    %% 1: first image in series (EPI-T2 decay)
else
