function saccs = saccadeAnalysis1D (em_list,samp_time,resamp_time)
...
VEL_THRESHOLD          = 3.0;   % Thresh velocity for "candidate" saccades
...
N_MONOTONIC	       = 15;     % number of continuous ms to identify saccade
VELCONVSTD	       = 3.0;   % milliseconds velocity gaussian smoothing
MINIMUM_AMPLITUDE      = 0.05;  % smallest detectable amplitude for the algorithm
MINIMUM_INTERVAL       = 10.0;  % smallest detectable interval
...
BLINK_VEL_THRESH       = 100;	% deg/sec, velocity threshold for detecting blinks
BLINK_DISP_THRESH      = 4.0;	% deg/sec, velocity threshold for detecting blinks
SHOW_FIGURES	       = 0;     % display some raw traces, distributions, etc
...
if SHOW_FIGURES
 ...
 for i=1:length(saccs.S_times)
   ...
   plot([start:stop],saccs.S_em(start)+form,'b'); 
   hold on
   if i < length(saccs.S_times)	 
      start2 = stop;
      stop2  = saccs.S_times(i+1)-FORM_MS_HWIN;
      len = stop2-start2+1;
      plot([start2:stop2],saccs.S_em(start2)+ tmpsaccs.C_postsaccfixslope(i)*[1:len],'r');
   end
 end
