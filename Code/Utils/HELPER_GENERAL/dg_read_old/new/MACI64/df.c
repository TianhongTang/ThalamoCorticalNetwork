 ...
 *       multiple times stored in the smilet entry. (It could be a int,
 ...
 *       If you only add a int or float entry, then no explicit freeing
 *       is required.
 *
...
TAG_INFO DFTags[] = {
  ...
  { D_NOBSP_TAG,  "NOBS_PERIODS",  DF_INT,       TOP_LEVEL },
  { D_OBSP_TAG,   "OBS_PERIOD",    DF_STRUCTURE,  OBS_P_STRUCT },
  { D_NCINFO_TAG, "NCELL_INFOS",   DF_INT,       TOP_LEVEL },
  { D_CINFO_TAG,  "CELL_INFO",     DF_STRUCTURE,  CELL_INFO_STRUCT },
};

TAG_INFO DFInfoTags[] = {
  { D_FILENAME_TAG,  "FILENAME",   DF_STRING, TOP_LEVEL },
  { D_TIME_TAG,      "TIME",       DF_INT,   TOP_LEVEL },
  { D_FILENUM_TAG,   "FILENUM",    DF_INT,   TOP_LEVEL },  
  { D_COMMENT_TAG,   "COMMENTS",   DF_STRING, TOP_LEVEL },
  { D_EXP_TAG,       "EXP_ID",     DF_INT,   TOP_LEVEL }, 
  { D_TMODE_TAG,     "TEST_MODE",  DF_INT,   TOP_LEVEL },
  { D_EMCOLLECT_TAG, "EM_ON",      DF_CHAR,   TOP_LEVEL },
  { D_SPCOLLECT_TAG, "SPIKE_ON",   DF_CHAR,   TOP_LEVEL },
  { D_NSTIMTYPES_TAG,"NSTIMTYPES", DF_INT,   TOP_LEVEL },
...
TAG_INFO ObsInfoTags[] = {
  { O_BLOCK_TAG,     "BLOCK_NUM",    DF_INT,   TOP_LEVEL },
  { O_OBSP_TAG,      "OBS_NUM",      DF_INT,   TOP_LEVEL }, 
  { O_STATUS_TAG,    "OBS_STATUS",   DF_INT,   TOP_LEVEL },
  { O_DURATION_TAG,  "DURATION",     DF_INT,   TOP_LEVEL },
  { O_NTRIALS_TAG,   "NTRIALS",      DF_INT,   TOP_LEVEL },
  { O_FILENUM_TAG,   "FILENUM",      DF_INT,   TOP_LEVEL },
  { O_INDEX_TAG,     "OBSINDEX",     DF_INT,   TOP_LEVEL },
};
...
TAG_INFO EvListTags[] = {
  { E_VAL_LIST_TAG,  "VALUES",  DF_INT_ARRAY,   TOP_LEVEL },
  { E_TIME_LIST_TAG, "TIMES",   DF_INT_ARRAY,   TOP_LEVEL },
};

TAG_INFO EMDataTags[] = {
  { E_ONTIME_TAG,    "EMSTART",   DF_INT,             TOP_LEVEL },
  ...
  { E_PNT_DEG_TAG,   "PNT_DEG",   DF_INT,             TOP_LEVEL },
...
TAG_INFO SPDataTags[] = {
  { S_NCHANNELS_TAG,  "NSP_CHANNELS",   DF_INT,       TOP_LEVEL },
...
TAG_INFO SPChannelTags[] = {
  ...
  { S_CH_CELLNUM_TAG, "CELLNUM",       DF_INT,        TOP_LEVEL },
};

TAG_INFO CellTags[] = {
  { C_NUM_TAG,       "CELLNUM",           DF_INT,             TOP_LEVEL },
...
/* record the number of cell_infos and then the structures */
  dfRecordInt(D_NCINFO_TAG, DF_NCINFO(df));
...
/* record the number of observation periods and then the structures */
  dfRecordInt(D_NOBSP_TAG, DF_NOBSP(df));
...
void dfRecordDFInfo(unsigned char tag, DF_INFO *dfinfo)
{
  ...
  dfRecordInt(D_TIME_TAG, DF_TIME(dfinfo));
  dfRecordInt(D_FILENUM_TAG, DF_FILENUM(dfinfo));
  dfRecordString(D_COMMENT_TAG, DF_COMMENT(dfinfo));
  dfRecordInt(D_EXP_TAG, DF_EXPERIMENT(dfinfo));
  dfRecordInt(D_TMODE_TAG, DF_TESTMODE(dfinfo));
  dfRecordInt(D_NSTIMTYPES_TAG, DF_NSTIMTYPES(dfinfo));
...
{
  dfBeginStruct(tag);
  dfRecordInt(O_FILENUM_TAG, OBS_FILENUM(oinfo));
  dfRecordInt(O_INDEX_TAG, OBS_INDEX(oinfo));
  dfRecordInt(O_BLOCK_TAG, OBS_BLOCK(oinfo));
  dfRecordInt(O_OBSP_TAG, OBS_OBSP(oinfo));
  dfRecordInt(O_STATUS_TAG, OBS_STATUS(oinfo));
  dfRecordInt(O_DURATION_TAG, OBS_DURATION(oinfo));
  dfRecordInt(O_NTRIALS_TAG, OBS_NTRIALS(oinfo));
  dfEndStruct();
}
...
void dfRecordEvList(unsigned char tag, EV_LIST *evlist)
{
  if (EV_LIST_N(evlist)) {
    dfBeginStruct(tag);
    dfRecordIntArray(E_VAL_LIST_TAG, 
		      EV_LIST_N(evlist), EV_LIST_VALS(evlist));
    dfRecordIntArray(E_TIME_LIST_TAG, 
...
{
  dfBeginStruct(tag);
  dfRecordInt(E_ONTIME_TAG, EM_ONTIME(emdata));
  ...
  dfRecordInt(E_PNT_DEG_TAG, EM_PNT_DEG(emdata));
...
void dfRecordSpData(unsigned char tag, SP_DATA *spdata)
{
  int i;
  dfBeginStruct(tag);
  dfRecordInt(S_NCHANNELS_TAG, SP_NCHANNELS(spdata));
...
{
  dfBeginStruct(tag);
  dfRecordInt(S_CH_CELLNUM_TAG, SP_CH_CELLNUM(chdata));
...
{
  dfBeginStruct(tag);
  dfRecordInt(C_NUM_TAG, CI_NUMBER(cinfo));
...
void dfRecordIntArray(unsigned char type, int n, int *a)
{
  send_event(type, (unsigned char *) &n);
...
void dfRecordInt(unsigned char type, int val)
{
  send_event(type, (unsigned char *) &val);
}
...
/* All other tags may have data; check the current struct tag table  */
  ...
  case DF_FLAG:			/* no data associated with a flag    */
    break;
  case DF_STRING:		/* all of these start w/a int   */
