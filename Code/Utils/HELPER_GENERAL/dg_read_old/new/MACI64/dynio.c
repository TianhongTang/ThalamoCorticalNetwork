/* DAL -- Changed long to int through entire code (because of 64-bit) */
/* 15-Sept-2007 */
...
TAG_INFO DLTags[] = {
  ...
  { DL_INT_DATA_TAG,   "INT_DATA",     DF_INT_ARRAY,   DG_TOP_LEVEL },
  ...
  { DL_FLAGS_TAG,       "FLAGS",       DF_INT,         DG_TOP_LEVEL }
};
...
void dgRecordDynList(unsigned char tag, DYN_LIST *dl)
{
  ...
  dgRecordInt(DL_INCREMENT_TAG, DYN_LIST_INCREMENT(dl));
  dgRecordInt(DL_FLAGS_TAG, DYN_LIST_FLAGS(dl));
...
void dgRecordDynGroup(DYN_GROUP *dg)
{
  ...
  dgRecordInt(DG_NLISTS_TAG, DYN_GROUP_NLISTS(dg));
...
void dgRecordVoidArray(unsigned char type, int datatype, int n, void *data)
{
  ...
  case DF_SHORT:
    ...
    break;
  case DF_INT:
    dgRecordIntArray(DL_INT_DATA_TAG, n, (int *) data);
    break;
  case DF_FLOAT:
...
void dgRecordIntArray(unsigned char type, int n, int *a)
{
  send_event(type, (unsigned char *) &n);
...
void dgRecordInt(unsigned char type, int val)
{
  send_event(type, (unsigned char *) &val);
}
...
/* All other tags may have data; check the current struct tag table  */
  ...
  case DF_VOID_ARRAY:
    break;
  case DF_STRING:		/* all of these start w/a int int   */
...
void read_int(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dgFlipEvents) val = flipint(val);
...
void read_string(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dgFlipEvents) length = flipint(length);
  if (length) {
...
void read_chars(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dgFlipEvents) nchars = flipint(nchars);
...
void read_ints(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (fread(&nints, sizeof(int), 1, InFP) != 1) {
    fprintf(stderr,"Error reading number of intss\n");
    exit(-1);
  }
  
  
  if (dgFlipEvents) nints = flipint(nints);
  ...
  if (nints) {
    ...
    if (dgFlipEvents) flipints(nints, vals);
  }

  fprintf(OutFP, "%-20s\t%d\n", dgGetTagName(type), nints); 
...
void read_shorts(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dgFlipEvents) nshorts = flipint(nshorts);
...
void read_floats(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dgFlipEvents) nfloats = flipint(nfloats);
...
int vread_ints(char type, int *n, FILE *OutFP)
{
  ...
  if (dgFlipEvents) nvals = flipint(nvals);
  ...
  if (nvals) {
    ...
    if (dgFlipEvents) flipints(nvals, vals);
  }
...
{
  int c, status = DF_OK, iteration =  0;
  float version;
...
{
  int n = 0, nlists, c, status = DF_OK, iteration = 0;
...
int dguFileToDynList(FILE *InFP, DYN_LIST *dl)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case DL_INCREMENT_TAG:
      get_int(InFP, (int *) &DYN_LIST_INCREMENT(dl));
      break;
    case DL_FLAGS_TAG:
      get_int(InFP, (int *) &DYN_LIST_FLAGS(dl));
      break;
    case DL_DATA_TAG:
      break;
    case DL_NAME_TAG:
    ...
    case DL_FLOAT_DATA_TAG:
      ...
      break;
    case DL_INT_DATA_TAG:
      {
        int *data;
        int n;
        get_ints(InFP, &n, &data);
        DYN_LIST_DATATYPE(dl) = DF_INT;
        DYN_LIST_MAX(dl) = n;
        DYN_LIST_N(dl) = n;
        if (n) DYN_LIST_VALS(dl) = data;
        else DYN_LIST_VALS(dl) = NULL;
...
static int dguBufferToDynGroup(BUF_DATA *bdata, DYN_GROUP *dg)
{
  ...
  while (status == DF_OK && !BD_EOF(bdata)) {
	...
	free((void *) string);
      ...
      break;
    case DG_NLISTS_TAG:
      advance_bytes += vget_int((int *) BD_DATA(bdata), &nlists);
      break;
    case DG_DYNLIST_TAG:
      {
	DYN_LIST *dl = (DYN_LIST *) calloc(1, sizeof(DYN_LIST));
    ...
    case DL_INT_DATA_TAG:
      {
        int *data;
        int n;
        advance_bytes += vget_ints((int *) BD_DATA(bdata), &n, &data);
        DYN_LIST_DATATYPE(dl) = DF_INT;
        DYN_LIST_MAX(dl) = n;
        DYN_LIST_N(dl) = n;
        if (n) DYN_LIST_VALS(dl) = data;
    ...
    case DL_LIST_DATA_TAG:
      {
        ...
        /* Figure out how many there are */
        advance_bytes = vget_int((int *) BD_DATA(bdata), (int *) &n);
        BD_INCINDEX(bdata, advance_bytes);
        advance_bytes = 0;
