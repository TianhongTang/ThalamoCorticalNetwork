...
void dgRecordVoidArray(unsigned char type, int datatype, int n, void *data)
{
  ...
  case DF_SHORT:
    ...
    break;
  case DF_LONG:
    dgRecordLongArray(DL_LONG_DATA_TAG, n, (long *) data);
    break;
  case DF_FLOAT:
...
void dgRecordLongArray(unsigned char type, int n, long *a)
{
  send_event(type, (unsigned char *) &n);
  send_bytes(n*sizeof(long), (unsigned char *) a);
}
...
void dgRecordLong(unsigned char type, long val)
{
  send_event(type, (unsigned char *) &val);
}
...
int vread_longs(char type, int *n, FILE *OutFP)
{
  ...
  if (nvals) {
    if (!(vals = (long *) calloc(nvals, sizeof(long)))) {
      fprintf(stderr,"dgutils: error allocating space for long array\n");
      exit(-1);
    }
...
int dguFileToDynList(FILE *InFP, DYN_LIST *dl)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case DL_INCREMENT_TAG:
      get_long(InFP, (long *) &DYN_LIST_INCREMENT(dl));
      break;
    case DL_FLAGS_TAG:
      get_long(InFP, (long *) &DYN_LIST_FLAGS(dl));
      break;
    case DL_DATA_TAG:
      break;
    case DL_NAME_TAG:
...
static int dguBufferToDynGroup(BUF_DATA *bdata, DYN_GROUP *dg)
{
  ...
  while (status == DF_OK && !BD_EOF(bdata)) {
	...
	free((void *) string);
      ...
      break;
    case DG_NLISTS_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), &nlists);
      break;
    case DG_DYNLIST_TAG:
      {
	DYN_LIST *dl = (DYN_LIST *) calloc(1, sizeof(DYN_LIST));
  ...
  while (status == DF_OK && !BD_EOF(bdata)) {
    ...
    case DL_INCREMENT_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), 
				 (long *) &DYN_LIST_INCREMENT(dl));
      break;
    case DL_FLAGS_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), 
				 (long *) &DYN_LIST_FLAGS(dl));
      break;
    case DL_DATA_TAG:
      break;
    case DL_NAME_TAG:
    ...
    case DL_LIST_DATA_TAG:
      {
        ...
        /* Figure out how many there are */
        advance_bytes = vget_long((long *) BD_DATA(bdata), (long *) &n);
        BD_INCINDEX(bdata, advance_bytes);
        advance_bytes = 0;
