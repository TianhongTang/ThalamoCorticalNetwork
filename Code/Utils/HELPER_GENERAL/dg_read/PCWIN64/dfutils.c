...
void read_long(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (fread(&val, sizeof(long), 1, InFP) != 1) {
...
void read_longs(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (nlongs) {
    if (!(vals = (long *) calloc(nlongs, sizeof(long)))) {
      fprintf(stderr,"Error allocating memory for long array\n");
      exit(-1);
    }
  
    if (fread(vals, sizeof(long), nlongs, InFP) != nlongs) {
...
static
int vread_long(char type, long *ival, FILE *OutFP)
{
  long val;
  memcpy(&val, ival, sizeof(long));
...
int vread_strings(char type, int *iptr, FILE *OutFP)
{
  int n, i;
  int length, sum = 0;
  char *next = (char *) iptr + sizeof(long);
  char *str;
  ...
  for (i = 0; i < n; i++) {
    ...
    str = "";
    if (length) str = (char *) next+sizeof(long);
    
    fprintf(OutFP, "%d\t%s\n", i, str);
    sum += length;
    next += sizeof(long)+length;
  }
  return(sum+(n*sizeof(long))+sizeof(long));
}
...
int vread_longs(char type, int *n, FILE *OutFP)
{
  ...
  if (nvals) {
    if (!(vals = (long *) calloc(nvals, sizeof(long)))) {
      fprintf(stderr,"dfutils: error allocating space for long array\n");
      exit(-1);
    }
    memcpy(vals, vl, sizeof(long)*nvals);
  ...
  if (vals) free(vals);
  return(sizeof(int)+nvals*sizeof(long));
}
...
{
  return(skip_bytes(InFP, sizeof(long)));
}
...
static int skip_longs(FILE *InFP)
{
  ...
  if (dfFlipEvents) nvals = fliplong(nvals);
  return(skip_bytes(InFP, nvals*sizeof(long)));
}
...
static 
void get_long(FILE *InFP,  long *ival)
{
  long val;
...
static
void get_longs(FILE *InFP, int *n, long **v)
{
  int nvals;
  long *vals = NULL;
  ...
  if (nvals) {
    ...
    if (fread(vals, sizeof(long), nvals, InFP) != nvals) {
...
static
int vget_long(long *ival, long *l)
{
  long val;
  memcpy(&val, ival, sizeof(long));
...
static
int vget_longs(int *n, int *nv, long **v)
{
  int nvals;
  int *next = n+1;
  long *vl = (long *) next;
...
int dfuFileToDataFile(FILE *InFP, DATA_FILE *df)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case D_DFINFO_TAG:
      ...
      break;
    case D_NOBSP_TAG:
      get_long(InFP, (long *) &DF_NOBSP(df));
	...
	status = DF_ABORT;
      ...
      break;
    case D_NCINFO_TAG:
      get_long(InFP, (long *) &DF_NCINFO(df));
      if (DF_NCINFO(df)) {
...
int dfuFileToDFInfo(FILE *InFP, DF_INFO *dfinfo)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case D_TIME_TAG:
      ...
      break;
    case D_FILENUM_TAG:
      get_long(InFP, (long *) &DF_FILENUM(dfinfo));
      break;
    case D_AUXFILES_TAG:
    ...
    case D_COMMENT_TAG:
      ...
      break;
    case D_EXP_TAG:
      get_long(InFP, (long *) &DF_EXPERIMENT(dfinfo));
      break;
    case D_TMODE_TAG:
      get_long(InFP, (long *) &DF_TESTMODE(dfinfo));
      break;
    case D_NSTIMTYPES_TAG:
      get_long(InFP, (long *) &DF_NSTIMTYPES(dfinfo));
      break;
...
int dfuFileToObsInfo(FILE *InFP, OBS_INFO *oinfo)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case END_STRUCT:
      ...
      break;
    case O_FILENUM_TAG:
      get_long(InFP, (long *) &OBS_FILENUM(oinfo));
      break;
    case O_INDEX_TAG:
      get_long(InFP, (long *) &OBS_INDEX(oinfo));
      break;
    case O_BLOCK_TAG:
      get_long(InFP, (long *) &OBS_BLOCK(oinfo));
      break;
    case O_OBSP_TAG:
      get_long(InFP, (long *) &OBS_OBSP(oinfo));
      break;
    case O_STATUS_TAG:
      get_long(InFP, (long *) &OBS_STATUS(oinfo));
      break;
    case O_DURATION_TAG:
      get_long(InFP, (long *) &OBS_DURATION(oinfo));
      break;
    case O_NTRIALS_TAG:
      get_long(InFP, (long *) &OBS_NTRIALS(oinfo));
      break;
    default:
      fprintf(stderr,"unknown event type %d\n", c);
      status = DF_ABORT;
...
int dfuFileToEmData(FILE *InFP, EM_DATA *emdata)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case END_STRUCT:    
      ...
      break;
    case E_ONTIME_TAG:
      get_long(InFP, (long *) &EM_ONTIME(emdata));
      break;
    case E_RATE_TAG:
	...
	status = DF_ABORT;
      ...
      break;
    case E_PNT_DEG_TAG:
      get_long(InFP, (long *) &EM_PNT_DEG(emdata));
      break;
...
int dfuFileToSpData(FILE *InFP, SP_DATA *spdata)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case S_NCHANNELS_TAG:
      get_long(InFP, (long *) &SP_NCHANNELS(spdata));
...
int dfuFileToSpChData(FILE *InFP, SP_CH_DATA *spchdata)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case S_CH_CELLNUM_TAG:
      get_long(InFP, (long *) &SP_CH_CELLNUM(spchdata));
      break;
...
int dfuFileToCellInfo(FILE *InFP, CELL_INFO *cinfo)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case END_STRUCT:    
      ...
      break;
    case C_NUM_TAG:
      get_long(InFP, (long *) &CI_NUMBER(cinfo));
      break;
    case C_DISCRIM_TAG:
...
int dfuBufferToDataFile(BUF_DATA *bdata, DATA_FILE *df)
{
  ...
  while (status == DF_OK && !BD_EOF(bdata)) {
    ...
    case D_DFINFO_TAG:
      ...
      break;
    case D_NOBSP_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), 
				 (long *) &DF_NOBSP(df));
      if (!DF_NOBSP(df)) DF_OBSPS(df) = NULL;
      else {
	DF_OBSPS(df) = (OBS_P *) calloc(DF_NOBSP(df), sizeof(OBS_P));
...
int dfuBufferToDFInfo(BUF_DATA *bdata, DF_INFO *dfinfo)
{
  ...
  while (status == DF_OK && !BD_EOF(bdata)) {
    ...
    case D_EXP_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), 
				 (long *) &DF_EXPERIMENT(dfinfo));
      break;
    case D_TMODE_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), 
				 (long *) &DF_TESTMODE(dfinfo));
      break;
    case D_NSTIMTYPES_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), 
				 (long *) &DF_NSTIMTYPES(dfinfo));
      break;
    case D_EMCOLLECT_TAG:
...
int dfuBufferToObsInfo(BUF_DATA *bdata, OBS_INFO *oinfo)
{
  ...
  while (status == DF_OK && !BD_EOF(bdata)) {
    ...
    case O_FILENUM_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), 
				 (long *) &OBS_FILENUM(oinfo));
      break;
    case O_INDEX_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), 
				 (long *) &OBS_INDEX(oinfo));
      break;
    case O_BLOCK_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), 
				 (long *) &OBS_BLOCK(oinfo));
      break;
    case O_OBSP_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), 
				 (long *) &OBS_OBSP(oinfo));
    ...
    case O_STATUS_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata),
				 (long *) &OBS_STATUS(oinfo));
      break;
    case O_DURATION_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), 
				 (long *) &OBS_DURATION(oinfo));
      break;
    case O_NTRIALS_TAG:
      advance_bytes += vget_long((long *) BD_DATA(bdata), 
				 (long *) &OBS_NTRIALS(oinfo));
      break;
    default:
      fprintf(stderr,"unknown event type %d\n", c);
      status = DF_ABORT;
...
void dfuBufferToAscii(unsigned char *vbuf, int bufsize, FILE *OutFP)
{
  ...
  for (i = DF_MAGIC_NUMBER_SIZE; i < bufsize; i+=advance_bytes) {
    ...
    case DF_CHAR:
      ...
      break;
    case DF_LONG:
      advance_bytes = vread_long(c, (long *) &vbuf[i], OutFP);
      break;
    case DF_SHORT:
...
DYN_LIST *dfuCreateNamedDynList(char *name, int datatype, int increment)
{
  ...
  case DF_LONG:
    DYN_LIST_VALS(dynlist) = 
      (long *)calloc(DYN_LIST_MAX(dynlist), sizeof(long));
    break;
  case DF_SHORT:
    DYN_LIST_VALS(dynlist) = 
...
DYN_LIST *dfuResetDynListToType(DYN_LIST *dynlist, int datatype, int increment)
{
  ...
  case DF_LONG:
    DYN_LIST_VALS(dynlist) = 
      (long *)realloc(DYN_LIST_VALS(dynlist), increment*sizeof(long));
    break;
  case DF_SHORT:
    DYN_LIST_VALS(dynlist) = 
...
/***********************************************************************
 *
 * dfuAddDynListLong(DYN_LIST *, long val)
 *
...
void dfuAddDynListLong(DYN_LIST *dynlist, long val)
{
  long *vals;
  if (!dynlist) return;
  ...
  if (DYN_LIST_N(dynlist) == DYN_LIST_MAX(dynlist)) {
    ...
    vals = (long *) realloc(vals, sizeof(long)*DYN_LIST_MAX(dynlist));
  }
...
/***********************************************************************
 *
 * dfuPrependDynListLong(DYN_LIST *, long val)
 *
...
void dfuPrependDynListLong(DYN_LIST *dynlist, long val)
{
...
/***********************************************************************
 *
 * dfuInsertDynListLong(DYN_LIST *, long val, int pos)
 *
...
int dfuInsertDynListLong(DYN_LIST *dynlist, long val, int pos)
{
  long *vals;
  int i;
 ...
 * are found in the event list, they are freed.
 *
 * NOTE:  The dynamic list must be of type DF_LONG (longs).
 *
 ***********************************************************************/
