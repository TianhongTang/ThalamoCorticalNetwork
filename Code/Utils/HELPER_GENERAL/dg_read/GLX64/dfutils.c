...
static
void read_int(char type, FILE *InFP, FILE *OutFP)
{
  int val;
  ...
  if (dfFlipEvents) val = flipint(val);
...
void read_string(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dfFlipEvents) length = flipint(length);
  if (length) {
...
static
void read_ints(char type, FILE *InFP, FILE *OutFP)
{
  int nints, i;
  int *vals = NULL;
  
  if (fread(&nints, sizeof(int), 1, InFP) != 1) {
    fprintf(stderr,"Error reading number of ints\n");
    exit(-1);
  }
  
  if (dfFlipEvents) nints = flipint(nints);
  
  if (nints) {
    if (!(vals = (int *) calloc(nints, sizeof(int)))) {
      fprintf(stderr,"Error allocating memory for int array\n");
      exit(-1);
    }
  
    if (fread(vals, sizeof(int), nints, InFP) != nints) {
      fprintf(stderr,"Error reading int array\n");
      exit(-1);
    }
  
    if (dfFlipEvents) flipints(nints, vals);
  }

  fprintf(OutFP, "%-20s\t%d\n", dfGetTagName(type), nints); 
...
void read_shorts(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dfFlipEvents) nshorts = flipint(nshorts);
...
void read_floats(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dfFlipEvents) nfloats = flipint(nfloats);
...
static
int vread_int(char type, int *ival, FILE *OutFP)
{
  int val;
...
static
int vread_ints(char type, int *n, FILE *OutFP)
{
  int i;
  int nvals;
  int *next = n+1;
  ...
  if (dfFlipEvents) nvals = flipint(nvals);
  ...
  if (nvals) {
    ...
    if (dfFlipEvents) flipints(nvals, vals);
  }
...
static int skip_ints(FILE *InFP)
{
  int nvals;
...
static 
void get_int(FILE *InFP,  int *ival)
{
  int val;
...
static
void get_ints(FILE *InFP, int *n, int **v)
{
  int nvals;
  int *vals = NULL;
  ...
  if (nvals) {
    if (!(vals = (int *) calloc(nvals, sizeof(int)))) {
      fprintf(stderr,"Error allocating memory for int elements\n");
      exit(-1);
    }
...
static
int vget_ints(int *n, int *nv, int **v)
{
  int nvals;
  int *next = n+1;
  int *vl = (int *) next;
...
int dfuFileToDFInfo(FILE *InFP, DF_INFO *dfinfo)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case D_FILENAME_TAG:
      ...
      break;
    case D_TIME_TAG:
      get_int(InFP, &DF_TIME(dfinfo));
      break;
    case D_FILENUM_TAG:
      get_int(InFP, (int *) &DF_FILENUM(dfinfo));
      break;
    case D_AUXFILES_TAG:
    ...
    case D_COMMENT_TAG:
      ...
      break;
    case D_EXP_TAG:
      get_int(InFP, (int *) &DF_EXPERIMENT(dfinfo));
      break;
    case D_TMODE_TAG:
      get_int(InFP, (int *) &DF_TESTMODE(dfinfo));
      break;
    case D_NSTIMTYPES_TAG:
      get_int(InFP, (int *) &DF_NSTIMTYPES(dfinfo));
      break;
...
int dfuFileToObsInfo(FILE *InFP, OBS_INFO *oinfo)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case END_STRUCT:
      ...
      break;
    case O_FILENUM_TAG:
      get_int(InFP, (int *) &OBS_FILENUM(oinfo));
      break;
    case O_INDEX_TAG:
      get_int(InFP, (int *) &OBS_INDEX(oinfo));
      break;
    case O_BLOCK_TAG:
      get_int(InFP, (int *) &OBS_BLOCK(oinfo));
      break;
    case O_OBSP_TAG:
      get_int(InFP, (int *) &OBS_OBSP(oinfo));
      break;
    case O_STATUS_TAG:
      get_int(InFP, (int *) &OBS_STATUS(oinfo));
      break;
    case O_DURATION_TAG:
      get_int(InFP, (int *) &OBS_DURATION(oinfo));
      break;
    case O_NTRIALS_TAG:
      get_int(InFP, (int *) &OBS_NTRIALS(oinfo));
      break;
    default:
      fprintf(stderr,"unknown event type %d\n", c);
      status = DF_ABORT;
...
int dfuFileToEvList(FILE *InFP, EV_LIST *evlist)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case END_STRUCT:    
      ...
      break;
    case E_VAL_LIST_TAG: 
      get_ints(InFP, &EV_LIST_N(evlist), &EV_LIST_VALS(evlist));
      break;
    case E_TIME_LIST_TAG: 
      get_ints(InFP, &EV_LIST_NTIMES(evlist), &EV_LIST_TIMES(evlist));
      break;
    default:
      fprintf(stderr,"unknown event type %d\n", c);
      status = DF_ABORT;
...
int dfuFileToEmData(FILE *InFP, EM_DATA *emdata)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case END_STRUCT:    
      ...
      break;
    case E_ONTIME_TAG:
      get_int(InFP, (int *) &EM_ONTIME(emdata));
      break;
    case E_RATE_TAG:
	...
	status = DF_ABORT;
      ...
      break;
    case E_PNT_DEG_TAG:
      get_int(InFP, (int *) &EM_PNT_DEG(emdata));
      break;
...
int dfuFileToSpData(FILE *InFP, SP_DATA *spdata)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case S_NCHANNELS_TAG:
      get_int(InFP, (int *) &SP_NCHANNELS(spdata));
...
int dfuFileToSpChData(FILE *InFP, SP_CH_DATA *spchdata)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case S_CH_CELLNUM_TAG:
      get_int(InFP, (int *) &SP_CH_CELLNUM(spchdata));
      break;
...
int dfuFileToCellInfo(FILE *InFP, CELL_INFO *cinfo)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case END_STRUCT:    
      ...
      break;
    case C_NUM_TAG:
      get_int(InFP, (int *) &CI_NUMBER(cinfo));
      break;
    case C_DISCRIM_TAG:
...
int dfuBufferToDataFile(BUF_DATA *bdata, DATA_FILE *df)
{
  ...
  while (status == DF_OK && !BD_EOF(bdata)) {
    ...
    case D_DFINFO_TAG:
      ...
      break;
    case D_NOBSP_TAG:
      advance_bytes += vget_int((int *) BD_DATA(bdata), 
				 (int *) &DF_NOBSP(df));
      if (!DF_NOBSP(df)) DF_OBSPS(df) = NULL;
      else {
	DF_OBSPS(df) = (OBS_P *) calloc(DF_NOBSP(df), sizeof(OBS_P));
...
int dfuBufferToEvList(BUF_DATA *bdata, EV_LIST *evlist)
{
  ...
  while (status == DF_OK && !BD_EOF(bdata)) {
    ...
    case END_STRUCT:    
      ...
      break;
    case E_VAL_LIST_TAG: 
      advance_bytes += vget_ints((int *) BD_DATA(bdata), 
...
void dfuBufferToAscii(unsigned char *vbuf, int bufsize, FILE *OutFP)
{
  ...
  for (i = DF_MAGIC_NUMBER_SIZE; i < bufsize; i+=advance_bytes) {
    ...
    case DF_CHAR:
      ...
      break;
    case DF_INT:
      advance_bytes = vread_int(c, (int *) &vbuf[i], OutFP);
      break;
    case DF_SHORT:
    ...
    case DF_FLOAT_ARRAY:
      ...
      break;
    case DF_INT_ARRAY:
      advance_bytes = vread_ints(c, (int *) &vbuf[i], OutFP);
      break;
    case DF_SHORT_ARRAY:
...
DYN_GROUP *dfuCreateDynEvData(void)
{
  ...
  for (i = 0; i < nlists; i++) 
    dfuAddDynGroupNewList(dg, "", DF_INT, 4);
...
{
  dfuAddDynListInt(DYN_GROUP_LIST(evgroup,type*2), val);
  dfuAddDynListInt(DYN_GROUP_LIST(evgroup,type*2+1), time);
}
			 ...
			 int p1, int p2, int p3, int p4)
{
  dfuAddDynListInt(DYN_GROUP_LIST(evgroup,type*2+1), time);
  dfuAddDynListInt(DYN_GROUP_LIST(evgroup,type*2), val);
  dfuAddDynListInt(DYN_GROUP_LIST(evgroup,type*2), p1);
  dfuAddDynListInt(DYN_GROUP_LIST(evgroup,type*2), p2);
  dfuAddDynListInt(DYN_GROUP_LIST(evgroup,type*2), p3);
  dfuAddDynListInt(DYN_GROUP_LIST(evgroup,type*2), p4);
}
...
/***********************************************************************
 *
 * dfuAddDynListInt(DYN_LIST *, int val)
 *
...
void dfuAddDynListInt(DYN_LIST *dynlist, int val)
{
  int *vals;
  if (!dynlist) return;
...
/***********************************************************************
 *
 * dfuPrependDynListInt(DYN_LIST *, int val)
 *
...
void dfuPrependDynListInt(DYN_LIST *dynlist, int val)
{
  dfuInsertDynListInt(dynlist, val, 0);
}

/***********************************************************************
 *
 * dfuInsertDynListInt(DYN_LIST *, int val, int pos)
 *
...
int dfuInsertDynListInt(DYN_LIST *dynlist, int val, int pos)
{
  int *vals;
  int i;
 ...
 * are found in the event list, they are freed.
 *
 * NOTE:  The dynamic list must be of type DF_INT (ints).
 *
 ***********************************************************************/

int dfuSetEvList(EV_LIST *evlist, DYN_LIST *vals, DYN_LIST *times)
{
  if (DYN_LIST_DATATYPE(vals) != DF_INT ||
      DYN_LIST_DATATYPE(times) != DF_INT) {
