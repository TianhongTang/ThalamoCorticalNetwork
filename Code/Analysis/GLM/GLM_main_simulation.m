...
simulation_states = {'Async', 'Sync', 'NoInput'};
states = {'RestOpen', 'RestClose', 'Task'};
alignments = {'Last'};

parameter_tag = 'Default';
kernel = 'DeltaPure';
reg = struct();
reg.l1=0;
reg.l2=0.2;
...
for control_idx = 1:length(controls)
    ...
    for session_idx = sessions
        for area_idx = 1:length(area_types)
            ...
            for simulation_state_idx = 1:length(simulation_states)
                simulation_state = simulation_states{simulation_state_idx};
                for align_idx = 1:length(alignments)
                    ...
                    for state_idx = 1:length(states)
                        ...
                        task.simulation_state = simulation_state;
                        task.alignment = alignment;
                        task.parameter_tag = parameter_tag;
                        task.state = 'Task'; 
                        task.name = [control, simulation_state, state, area_type, 'Align', alignment, parameter_tag];
