...
dataset_names = {'SlayerMus', 'SlayerSal', 'SlayerNoinj',...
                 ...
                 'EmperorMus', 'EmperorSal', 'EmperorNoinj', 'KZ'};
dataset_num = length(dataset_names);
                 
states = {'Task', 'RestOpen', 'RestClose'};
state_names = {'Task', 'Eye Open', 'Eye Closed'};
...
for dataset_idx = 1:dataset_num
    ...
    fprintf('Dataset %d/%d: %s\n', dataset_idx, dataset_num, dataset_name);
    ...
    for session_idx = 1:session_num
        ...
        for state_idx = 1:n_states
            ...
            load(raster_path, 'rasters', 'N', 'trial_num', 'cell_area');

            thalamus_filter = cellfun(@(x) strcmp(x, 'ThalamusAnterior'), cell_area);
