...
% Select one x/y condition pair, one scatter type, and one rectangular region.
% The script plots the full scatter and a zoomed scatter of points inside the region.
% If text_out = true, selected connection metadata are printed to the MATLAB command window.
...
% Region to inspect, in plotted coordinates.
x_range = [2, 3];
y_range = [2, 3];
...
%% Pool selected x/y pair across sessions
...
for session_i = 1:numel(meta_array)
    ...
    try
        state_x = load_state_connectivity(root, meta, x_prepost, x_state, x_kernel_idx);
        state_y = load_state_connectivity(root, meta, y_prepost, y_state, y_kernel_idx);
...
function data = make_pair_vectors(state_x, state_y, err_multi, meta, session_label)
    ...
    [from12, to12] = make_connection_index_vectors(state_x.idx1, state_x.idx2);
    [from21, to21] = make_connection_index_vectors(state_x.idx2, state_x.idx1);
    ...
    from_cell_idx = [from12; from21];
    to_cell_idx = [to12; to21];
    ...
    data.from_cell_idx = from_cell_idx;
    data.to_cell_idx = to_cell_idx;
    data.from_area = from_area;
    data.to_area = to_area;
end

function [from_vec, to_vec] = make_connection_index_vectors(from_idx, to_idx)
    [from_grid, to_grid] = ndgrid(from_idx(:), to_idx(:));
    from_vec = from_grid(:);
    to_vec = to_grid(:);
end
...
function print_selected_connections(selected_data)
    ...
    for i = 1:numel(selected_data.x_plot)
        fprintf('plot_x=%.6g, plot_y=%.6g, raw_x=%.6g, raw_y=%.6g, session="%s", %s%d -> %s%d, direction=%s, group=%s\n', ...
            selected_data.x_plot(i), selected_data.y_plot(i), ...
            selected_data.x_raw(i), selected_data.y_raw(i), ...
            selected_data.session_label(i), ...
            selected_data.from_area(i), selected_data.from_cell_idx(i), ...
            selected_data.to_area(i), selected_data.to_cell_idx(i), ...
...
function write_selected_connections_to_file(selected_data, text_path)
    ...
    fprintf(fid, 'plot_x\tplot_y\traw_x\traw_y\tsession\tfrom_area\tfrom_cell_idx\tto_area\tto_cell_idx\tdirection\tgroup\n');
    for i = 1:numel(selected_data.x_plot)
        fprintf(fid, '%.12g\t%.12g\t%.12g\t%.12g\t%s\t%s\t%d\t%s\t%d\t%s\t%s\n', ...
            selected_data.x_plot(i), selected_data.y_plot(i), ...
            selected_data.x_raw(i), selected_data.y_raw(i), ...
            selected_data.session_label(i), ...
            selected_data.from_area(i), selected_data.from_cell_idx(i), ...
