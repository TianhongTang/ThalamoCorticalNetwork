...
config.output.simulation_tag = sprintf('%s_s%d_%s_%s_%s', ...
    ...
    config.tc_source);
config.output.animal_name = sprintf('Sim%s', config.reference.animal_name);
...
progress_log('CONFIG', ...
    'network_source=%s, tc_source=%s, kernel=%s, reg=%s.', ...
...
function [sim_meta, network, kernels, source_info] = ...
    ...
    post_raster_info = load_reference_raster_cell_info(root, post_raster_meta);
    ...
    cell_info = build_cell_layout(config, cortical_source, post_raster_info);
    N = cell_info.N_total;
    ...
    if isempty(dt) || ~isscalar(dt) || ~isfinite(dt) || dt <= 0
        error(['Simulation dt could not be resolved. Set config.simulation.dt ' ...
               'to a positive value in seconds.']);
    end
    ...
    source_info = struct();
    source_info.post_raster_path = post_raster_info.source_path;
    ...
    progress_log('NETWORK', ...
        ['Network ready: N=%d, cortex=%d, thalamus=%d, ' ...
         'connection kernels=%d, PS kernels=%d, dt=%g s.'], ...
        sim_meta.N_total, sim_meta.N_cortex, sim_meta.N_thalamus, ...
        K_conn, K_ps, dt);
end
...
function cell_info = build_cell_layout(config, cortical_source, raster_source)
    ...
    cell_info.reference_raster_meta = raster_source.meta;
...
function meta = build_output_raster_meta(config, result, condition_i)
    condition = result.condition;
    output_session_idx = config.reference.session_idx * ...
