...
base_plot_params.n_col = 6;
base_plot_params.category_labels = category_labels;
...
run_issue_log_filename = fullfile(output_root, 'run_issue_log_plot_from_precomputed_category_pairwise.txt');

for hp_i = 1:numel(hyperparam_configs)
    try
        hp = fill_default_hyperparams(hyperparam_configs(hp_i));
        hp_title = make_hyperparam_title(hp);
        cache_file = fullfile(cache_root, make_hyperparam_folder_name(hp), 'precomputed_category_pairwise.mat');
        ...
        params.category_labels = base_plot_params.category_labels;
