...
% 2x2 categorical tables: ignore non-significant connections and compare only negative/positive.
cat_counts_pre_2x2 = cat_counts_pre_total([1, 3], [1, 3]);
agreement_pre_2x2 = compute_raw_agreement(cat_counts_pre_2x2);
kappa_pre_2x2 = compute_cohen_kappa(cat_counts_pre_2x2);
cat_n_pre_2x2 = sum(cat_counts_pre_2x2(:));

cat_counts_post_2x2 = cat_counts_post_total([1, 3], [1, 3]);
agreement_post_2x2 = compute_raw_agreement(cat_counts_post_2x2);
kappa_post_2x2 = compute_cohen_kappa(cat_counts_post_2x2);
cat_n_post_2x2 = sum(cat_counts_post_2x2(:));
...
% Row 1 = old column 1; Row 2 = old column 2; Row 3 = old column 3; Row 4 = old column 4.
...
plot_open_close_scatter(ax, data_pre_all, 'all_signed', ...
    scatter_marker_size, scatter_alpha, sprintf('Pooled Pre all connections, sessions=%d', valid_session_count));
...
plot_open_close_density(ax, data_pre_all.x_orig, data_pre_all.y_orig, ...
    ...
    sprintf('Pooled Pre all-connection density, sessions=%d', valid_session_count), 'signed');
...
plot_open_close_scatter(ax, data_post_all, 'all_signed', ...
    scatter_marker_size, scatter_alpha, sprintf('Pooled Post all connections, sessions=%d', valid_session_count));
...
plot_open_close_density(ax, data_post_all.x_orig, data_post_all.y_orig, ...
    ...
    sprintf('Pooled Post all-connection density, sessions=%d', valid_session_count), 'signed');
...
function data = empty_connection_data()
    ...
    data.x_either_sig = [];
    data.y_either_sig = [];
    data.open_cat_either_sig = [];
    data.close_cat_either_sig = [];

    data.x_both_sig = [];
    data.y_both_sig = [];
    data.open_cat_both_sig = [];
    data.close_cat_both_sig = [];
end
...
function [data, label_text] = make_open_close_vectors(open_state, close_state, err_multi)
    ...
    open_pos = x >  err_multi * xerr;
    open_neg = x < -err_multi * xerr;
    close_pos = y >  err_multi * yerr;
    close_neg = y < -err_multi * yerr;

    open_cat = zeros(size(x));
    close_cat = zeros(size(y));
    open_cat(open_pos) = 1;
    open_cat(open_neg) = -1;
    close_cat(close_pos) = 1;
    close_cat(close_neg) = -1;

    open_sig = open_cat ~= 0;
    close_sig = close_cat ~= 0;

    same_pos = open_pos & close_pos;
    same_neg = open_neg & close_neg;
    switch_posneg = open_pos & close_neg;
    switch_negpos = open_neg & close_pos;
    switch_any = switch_posneg | switch_negpos;

    either_sig = open_sig | close_sig;
    ...
    data.open_cat_orig = open_cat;
    data.close_cat_orig = close_cat;
    ...
    data.open_cat_either_sig = open_cat(either_sig);
    data.close_cat_either_sig = close_cat(either_sig);

    data.x_both_sig = x(both_sig);
    data.y_both_sig = y(both_sig);
    data.open_cat_both_sig = open_cat(both_sig);
    data.close_cat_both_sig = close_cat(both_sig);
...
function plot_open_close_scatter(ax, data, plot_mode, marker_size, marker_alpha, title_text)
    ...
    switch plot_mode
        ...
        case 'either_sig_signed'
            x = data.x_either_sig;
            y = data.y_either_sig;
            open_sig = data.open_cat_either_sig ~= 0;
            close_sig = data.close_cat_either_sig ~= 0;
            open_only = open_sig & ~close_sig;
            close_only = ~open_sig & close_sig;
...
function plot_open_close_density(ax, x, y, rho, pval, n_valid, nbin, clip_percentile, use_log_count, title_text, axis_mode)
...
function plot_category_transition_table(ax, counts, category_labels, agreement, kappa, n_valid, title_text)
    ...
    for y_idx = 1:n_cat
        for x_idx = 1:n_cat
            ...
            if this_ratio < 0.2  % threshold for switching text color for better visibility
                text_color = 'w';
            else
                text_color = 'k';
            end
