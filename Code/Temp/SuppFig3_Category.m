...
% Generates categorical comparison figures for each hyperparameter set.
%
% Metadata behavior:
...
for hp_i = 1:numel(hyperparam_configs)
    ...
    try
        ...
        progress_log('HP', '[%d/%d][1/4] Matched %d complete sessions; figures=%d; output=%s', ...
        ...
        if isempty(meta_array)
            ...
            progress_log('HP', '[%d/%d] SKIPPED: no indexed sessions.', ...
                hp_i, numel(hyperparam_configs));
            continue;
        end
        ...
        for session_i = 1:numel(meta_array)
            ...
            try
                ...
                loaded_states = load_all_required_states( ...
                    root, meta, kernel_indices, params.err_multi);
...
progress_log('SCRIPT', ...
    'Finished in %.1f s. Valid-session uses=%d, failed-session uses=%d, figures=%d.', ...
...
function text_out = condition_to_string(cond)
    text_out = sprintf('%s/%s/K%d', cond.prepost, cond.state, cond.kernel_idx);
end
