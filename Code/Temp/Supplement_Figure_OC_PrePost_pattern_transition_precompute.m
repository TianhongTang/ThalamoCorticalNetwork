...
% Precompute-only version.
% Loads metadata and model data, pools the required results for each
% hyperparameter configuration, and saves them into MAT files.
% No figures are rendered in this script.
...
cache_root = fullfile(root, 'Data', 'Working', 'FigureCache', 'SuppFig_OC_PrePost_pattern_transition');
check_path(cache_root);
run_issue_log = empty_run_issue_log();
run_issue_log_filename = fullfile(cache_root, 'run_issue_log_precompute.txt');
...
for hp_i = 1:numel(hyperparam_configs)
    try
        ...
        cache_folder = fullfile(cache_root, make_hyperparam_folder_name(hp));
        check_path(cache_folder);
        cache_file = fullfile(cache_folder, 'precomputed_compound_pattern.mat');
        ...
        for session_i = 1:numel(meta_array)
            ...
            try
                ...
                for fig_i = 1:n_fig
                    ...
                    k = cfg.kernel_idx;
                    if strcmp(cfg.analysis_kind, 'pre_to_post_openclose')
                        states = cfg.states;
                        [counts9, ~, counts4, ~] = make_openclose_pattern_transition_counts( ...
                            loaded_states.(state_key('Pre', 'RestOpen',  k)), ...
                            loaded_states.(state_key('Pre', 'RestClose', k)), ...
                            loaded_states.(state_key('Post', 'RestOpen', k)), ...
                            loaded_states.(state_key('Post', 'RestClose', k)), ...
                            params.err_multi);
                    elseif strcmp(cfg.analysis_kind, 'open_vs_close_prepost')
                        states = cfg.states; %#ok<NASGU>
                        [counts9, ~, counts4, ~] = make_prepost_pattern_openclose_counts( ...
                            loaded_states.(state_key('Pre',  'RestOpen',  k)), ...
                            loaded_states.(state_key('Post', 'RestOpen',  k)), ...
                            loaded_states.(state_key('Pre',  'RestClose', k)), ...
                            loaded_states.(state_key('Post', 'RestClose', k)), ...
                        ...
                        error('Unknown analysis_kind: %s', cfg.analysis_kind);
                    end
        ...
        if valid_session_count == 0
            message = 'No valid sessions were loaded. Skipping this hyperparameter set.';
            warning('%s', message);
        ...
        precomputed = struct();
        precomputed.hp = hp;
        precomputed.params = params;
        precomputed.figure_configs = figure_configs;
        precomputed.pooled = pooled;
        precomputed.valid_session_count = valid_session_count;
        precomputed.failed_session_count = failed_session_count;
        precomputed.cache_created_at = datestr(now);
        precomputed.source_script = mfilename('fullpath');
        save(cache_file, 'precomputed', '-v7.3');
        fprintf('Saved precomputed data: %s\n', cache_file);

    catch ME
        run_issue_log = append_run_issue(run_issue_log, hp_i, sprintf('set_%d', hp_i), 'Exception', ME.message);
        write_run_issue_log(run_issue_log_filename, run_issue_log);
        warning('Skipping hyperparameter configuration %d after error: %s', hp_i, ME.message);
        continue;
    end
end
