...
if text_out
    ...
    print_overlap_summary(plot_data_alt, highlight_mask_alt);
end
...
if save_text_out
    ...
    fprintf('Saved primary selected-connection text to:\n%s\n', text_path);

    alt_text_path = fullfile(save_folder, sprintf('Figure3_region_selection_overlap_%s.txt', sanitize_filename(alt_plot_type)));
    write_selected_connections_to_file(subset_plot_data(plot_data_alt, highlight_mask_alt), alt_text_path);
    fprintf('Saved alternate matched-connection text to:\n%s\n', alt_text_path);
end
...
function print_selected_connections(selected_data)
    [~, order] = sort(selected_data.y_plot, 'descend', 'MissingPlacement', 'last');

    fprintf('\n===== Selected connections: n = %d; sorted by plot_y descending =====\n', numel(selected_data.x_plot));
...
function print_overlap_summary(plot_data_alt, highlight_mask_alt)
    selected_alt = subset_plot_data(plot_data_alt, highlight_mask_alt);
    fprintf('\n===== Alternate plot matched connections: n = %d =====\n', numel(selected_alt.x_plot));
    if isempty(selected_alt.x_plot)
        return;
    end
    print_selected_connections(selected_alt);
end
