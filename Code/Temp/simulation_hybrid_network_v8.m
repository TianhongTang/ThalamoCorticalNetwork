%% simulation_hybrid_network_v8.m
...
% Each session receives base_seed + (session_i-1)*session_seed_stride.
...
% -------------------------------------------------------------------------
% OUTPUT IDENTITY
% All conditions generated for one simulated session share the same
% session_idx and are distinguished by meta.state. Full/Cortex views are
% distinguished by meta.area. Multiple sessions use consecutive indices.
% -------------------------------------------------------------------------
...
function config = default_simulation_config()
    ...
    config.output.align = 'Last';
    config.output.base_session_idx = 1;
    config.output.resting_dur_threshold = 15;
    config.output.overwrite = false;
    config.output.save_project_raster = true;
    config.output.save_project_border = true;
    config.output.save_manifest = true;
