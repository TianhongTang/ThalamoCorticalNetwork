%% Supplement Figure 3: categorical comparison matrices
% Precompute-only version.
% Loads metadata and model data, pools the required categorical results for
% each hyperparameter configuration, and saves them into MAT files.
...
%% Analysis parameters
err_multi = 1;
network_err_multi = 2; %#ok<NASGU>
density_nbin = 60; %#ok<NASGU>
scatter_marker_size = 8; %#ok<NASGU>
scatter_alpha = 0.25; %#ok<NASGU>
density_clip_percentile = [0.5, 99.5]; %#ok<NASGU>
density_use_log_count = true; %#ok<NASGU>
...
cache_root = fullfile(root, 'Data', 'Working', 'FigureCache', 'SuppFig3_CategoryOnly_OEResidual');
...
for hp_i = 1:numel(hyperparam_configs)
    try
        ...
        cache_file = fullfile(cache_folder, 'precomputed_category_pairwise.mat');
