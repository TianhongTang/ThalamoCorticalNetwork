...
% Edit the parameter block below to change datasets, states, areas, alignment,
% resting threshold, training config, and missing-file behavior.
...
task_groups = struct( ...
    'task_name', {'Slayer', 'Zeppelin', 'Emperor'}, ...
    'dataset_indices', {[1, 2, 3], [4, 5, 6], [7, 8, 9]} ...
);
...
training_config.kernel = 'DeltaPure';
...
function task = make_task_struct(dataset_name, session_idx, merge_type, prepost, state, task_params, training_config)
    ...
    task.align = task_params.align;
    task.session_idx = session_idx;
    task.dataset_name = sprintf('%s%s%s%s%s', dataset_name, prepost, state, merge_type, task_params.align);
