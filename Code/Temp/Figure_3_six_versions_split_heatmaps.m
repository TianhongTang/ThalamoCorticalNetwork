...
function render_comparison_figure(root, cfg, pooled_one, params)
    render_main_panels_figure(root, cfg, pooled_one, params);
    render_heatmap_panels_figure(root, cfg, pooled_one, params);
end

function render_main_panels_figure(root, cfg, pooled_one, params)
    % Output 1: non-heatmap panels only, kept as vector.
    main_n_row = 4;
    main_n_col = 3;

    f = figure('Color', 'w', 'Visible', params.figure_visible);
    tiledlayout(main_n_row, main_n_col, 'TileSpacing', 'Compact', 'Padding', 'Compact');
    tile_idx = @(row, col) rowcol_to_panel_index(row, col, main_n_col);
    ...
    for ctx_i = 1:numel(cfg.contexts)
        ...
        [pearson_r_orig, pearson_p_orig, spearman_rho_orig, spearman_p_orig, ~] = correlation_stats(data.x_orig, data.y_orig);
        ...
        add_panel_label(ax, row_top, 1, main_n_col);

        ax = nexttile(tile_idx(row_bottom, 1));
        plot_pair_density(ax, data.x_orig, data.y_orig, pearson_r_orig, pearson_p_orig, spearman_rho_orig, spearman_p_orig, ...
            params.density_nbin, params.density_clip_percentile, params.density_use_log_count, ...
            sprintf('%s all-connection density, sessions=%d', ctx.name, pooled_one.valid_session_count), ...
            'signed', params.colors, ctx.x.axis_label, ctx.y.axis_label, orig_axis_limit);
        add_panel_label(ax, row_bottom, 1, main_n_col);
        ...
        plot_pair_scatter(ax, data, 'either_sig_signed', params.scatter_marker_size, params.scatter_alpha, ...
            ...
            params.show_inset_stat, params.show_identity_line, params.show_fit_line, params.fit_line_method);
        add_panel_label(ax, row_top, 2, main_n_col);
        ...
        plot_pair_scatter(ax, data, 'both_sig_signed', params.scatter_marker_size, params.scatter_alpha, ...
            ...
            ctx.x.axis_label, ctx.y.axis_label, [], ...
            params.show_inset_stat, params.show_identity_line, params.show_fit_line, params.fit_line_method);
        add_panel_label(ax, row_bottom, 2, main_n_col);
        ...
        plot_pair_scatter(ax, data, 'same_abs', params.scatter_marker_size, params.scatter_alpha, ...
            ...
            ctx.x.axis_label, ctx.y.axis_label, [], ...
            params.show_inset_stat, params.show_identity_line, params.show_fit_line, params.fit_line_method);
        add_panel_label(ax, row_top, 3, main_n_col);
        ...
        plot_pair_scatter(ax, data, 'switch_abs', params.scatter_marker_size, params.scatter_alpha, ...
            ...
            ctx.x.axis_label, ctx.y.axis_label, [], ...
            params.show_inset_stat, params.show_identity_line, params.show_fit_line, params.fit_line_method);
        add_panel_label(ax, row_bottom, 3, main_n_col);
    end

    sgtitle(sprintf('%s | main panels', cfg.figure_title), 'Interpreter', 'none');

    save_folder = fullfile(root, 'Figures', 'Paper');
    check_path(save_folder);

    figWidth = 4 * main_n_col;
    figHeight = 4 * main_n_row;
    resolution = 300;
    ...
    preview_filename = fullfile(save_folder, [cfg.output_stub, '_main_preview.jpg']);
    exportgraphics(f, preview_filename, 'ContentType', 'image', 'BackgroundColor', 'white', 'Resolution', resolution);

    pdf_filename = fullfile(save_folder, [cfg.output_stub, '_main.pdf']);
    exportgraphics(f, pdf_filename, 'ContentType', 'vector', 'BackgroundColor', 'white', 'Resolution', resolution);

    close(f);
end

function render_heatmap_panels_figure(root, cfg, pooled_one, params)
    % Output 2: categorical heatmaps only, rasterized for Illustrator stability.
    heat_n_row = numel(cfg.contexts);
    heat_n_col = 2;

    f = figure('Color', 'w', 'Visible', params.figure_visible);
    tiledlayout(heat_n_row, heat_n_col, 'TileSpacing', 'Compact', 'Padding', 'Compact');
    tile_idx = @(row, col) rowcol_to_panel_index(row, col, heat_n_col);
    ...
    for ctx_i = 1:numel(cfg.contexts)
        ...
        add_panel_label(ax, row_i, 1, heat_n_col);

        ax = nexttile(tile_idx(row_i, 2));
        plot_category_transition_table(ax, counts_2x2, category_labels_2x2, agreement_2x2, kappa_2x2, n_cat_2x2, ...
            ...
            sprintf('%s category', ctx.x.axis_label), sprintf('%s category', ctx.y.axis_label));
        add_panel_label(ax, row_i, 2, heat_n_col);
    end

    sgtitle(sprintf('%s | categorical heatmaps', cfg.figure_title), 'Interpreter', 'none');

    save_folder = fullfile(root, 'Figures', 'Paper');
    check_path(save_folder);

    figWidth = 5 * heat_n_col;
    figHeight = 5 * heat_n_row;
    resolution = 600;
    ...
    png_filename = fullfile(save_folder, [cfg.output_stub, '_heatmaps.png']);
    exportgraphics(f, png_filename, 'ContentType', 'image', 'BackgroundColor', 'white', 'Resolution', resolution);

    pdf_filename = fullfile(save_folder, [cfg.output_stub, '_heatmaps.pdf']);
    exportgraphics(f, pdf_filename, 'ContentType', 'image', 'BackgroundColor', 'white', 'Resolution', resolution);

    close(f);
end

function panel_index = rowcol_to_panel_indexfunction panel_index = rowcol_to_panel_index(row, col, n_col)
