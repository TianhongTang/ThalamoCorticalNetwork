%% Supplement Figure 3: pooled categorical comparison matrices
% Pools multiple kernel_name models into configurable kernel setups and
% generates categorical comparison figures for each setup x injection x align.
%
% Metadata behavior:
%   1. On the first run, load the full GLM metadata as a struct, retain
%      epoch = 3000 and fixed analysis filters, verify complete
%      Pre/Post x RestOpen/RestClose sessions, and save a compact index.
%   2. On later runs, load only the compact index. The full metadata file is
%      not loaded.
%
% Figure versions for each pooled kernel setup:
%   1. Open vs Close, split by Pre/Post.
%   2. Pre vs Post, split by Open/Close.
%
% Layout:
%   Row 1 = context A, 3x3 table.
%   Row 2 = context A, 2x2 table excluding non-significant connections.
%   Row 3 = context B, 3x3 table.
%   Row 4 = context B, 2x2 table excluding non-significant connections.
%
% Columns:
%   1. Raw count.
%   2. Normalized by row.
%   3. Normalized by column.
%   4. Difference = counts - counts'.
%   5. Normalized difference = (counts-counts')/(counts+counts').
%   6. O/E ratio under the independent-marginal baseline.
%   7. Pearson residual = (O-E)/sqrt(E).
%
% Difference, normalized-difference, and residual panels use a zero-centered
% diverging colormap: brewermap(256, '*RdBu').
...
if exist('brewermap', 'file') == 0
    error('brewermap is required but was not found on the MATLAB path.');
end
...
diverging_cmap = brewermap(256, '*RdBu');
...
max_sessions_to_include = inf; % Limits kernel-session records; debug use only.

% Independent export switches. Default: JPG only.
export_jpg = true;
export_pdf = false;
export_resolution = 300;

% Metadata index controls. This is shared with the other Figure 3 script.
...
base_params.category_labels_2x2 = category_labels_2x2;
base_params.diverging_cmap = diverging_cmap;
...
for hp_i = 1:numel(hyperparam_configs)
    ...
    try
        ...
        progress_log('HP', '[%d/%d] START: %s', ...
            hp_i, numel(hyperparam_configs), params.hyperparam_title);
        progress_log('HP', '[%d/%d][1/4] Selecting sessions from compact index.', ...
            hp_i, numel(hyperparam_configs));

        figure_configs = build_figure_configs();
        ...
        progress_log('HP', ...
            '[%d/%d][1/4] Matched %d kernel-session records; figures=%d; output=%s', ...
            hp_i, numel(hyperparam_configs), numel(meta_array), ...
            numel(figure_configs), params.output_folder);
        if isempty(meta_array)
            ...
            run_issue_log = append_run_issue(run_issue_log, hp_i, ...
                params.hyperparam_title, 'NoMetadata', message);
            write_run_issue_log(run_issue_log_filename, run_issue_log);
            progress_log('HP', '[%d/%d] SKIPPED: no indexed kernel-session records.', ...
                hp_i, numel(hyperparam_configs));
            continue;
        end
        ...
        progress_log('HP', '[%d/%d][2/4] Initializing %d pooled figure structures.', ...
            hp_i, numel(hyperparam_configs), n_fig);
        pooled = struct([]);
        for fig_i = 1:n_fig
            n_contexts = numel(figure_configs(fig_i).contexts);
            pooled(fig_i).contexts = struct([]);
            for ctx_i = 1:n_contexts
                pooled(fig_i).contexts(ctx_i).cat_counts = zeros(3, 3);
            end
        ...
        for session_i = 1:numel(meta_array)
            ...
            try
                % Each of the four condition files is loaded once. All kernel
                % blocks are extracted and classified during that load.
                loaded_states = load_all_required_states( ...
                    root, meta, model_kernel_idx, params.err_multi);

                % Keep all results temporary until the entire session has
                % completed, preventing partial commits after a later error.
                session_counts = cell(n_fig, 1);
                for fig_i = 1:n_fig
                    cfg = figure_configs(fig_i);
                    n_contexts = numel(cfg.contexts);
                    session_counts{fig_i} = cell(1, n_contexts);

                    for ctx_i = 1:n_contexts
                        cond_x = cfg.contexts(ctx_i).x;
                        cond_y = cfg.contexts(ctx_i).y;
                        progress_log('ANALYSIS', ...
                            ['[HP %d/%d][Session %d/%d][Figure %d/%d]' ...
                             '[Context %d/%d] %s: %s vs %s.'], ...
                            hp_i, numel(hyperparam_configs), session_i, numel(meta_array), ...
                            fig_i, n_fig, ctx_i, n_contexts, cfg.contexts(ctx_i).name, ...
                            condition_to_string(cond_x), condition_to_string(cond_y));

                        state_x = loaded_states.(state_key( ...
                            cond_x.prepost, cond_x.state, cond_x.kernel_idx));
                        state_y = loaded_states.(state_key( ...
                            cond_y.prepost, cond_y.state, cond_y.kernel_idx));

                        session_counts{fig_i}{ctx_i} = ...
                            make_pair_category_counts(state_x, state_y);
                    end
                end

                for fig_i = 1:n_fig
                    for ctx_i = 1:numel(session_counts{fig_i})
                        pooled(fig_i).contexts(ctx_i).cat_counts = ...
                            pooled(fig_i).contexts(ctx_i).cat_counts + ...
                            session_counts{fig_i}{ctx_i};
                    end
                end
                ...
                if params.skip_failed_sessions
                    warning('Skipping %s because loading/processing failed: %s', ...
                        session_label, ME_session.message);
                    continue;
                else
                    rethrow(ME_session);
                end
        ...
        if valid_session_count == 0
            message = sprintf(['No valid kernel-session records were loaded. Failed sessions: %d. ' ...
                'Skipping rendering for this hyperparameter set.'], failed_session_count);
            warning('%s Hyperparameter set %d: %s', message, hp_i, params.hyperparam_title);
            run_issue_log = append_run_issue(run_issue_log, hp_i, ...
                params.hyperparam_title, 'NoValidSessions', message);
            write_run_issue_log(run_issue_log_filename, run_issue_log);
            progress_log('HP', '[%d/%d] SKIPPED: no valid kernel-session records.', ...
                hp_i, numel(hyperparam_configs));
            continue;
        end
        ...
        progress_log('HP', '[%d/%d][4/4] Rendering %d figures.', ...
            hp_i, numel(hyperparam_configs), n_fig);
        for fig_i = 1:n_fig
            progress_log('FIGURE', '[HP %d/%d][%d/%d] START: %s', ...
                hp_i, numel(hyperparam_configs), fig_i, n_fig, ...
                figure_configs(fig_i).output_stub);
            render_comparison_figure(figure_configs(fig_i), pooled(fig_i), params);
            total_rendered_figure_count = total_rendered_figure_count + 1;
            progress_log('FIGURE', '[HP %d/%d][%d/%d] DONE: %s', ...
                hp_i, numel(hyperparam_configs), fig_i, n_fig, ...
                figure_configs(fig_i).output_stub);
        end

        progress_log('HP', '[%d/%d] DONE in %.1f s. Valid=%d, failed=%d, figures=%d.', ...
            hp_i, numel(hyperparam_configs), toc(hp_tic), ...
            valid_session_count, failed_session_count, n_fig);
        ...
        run_issue_log = append_run_issue(run_issue_log, hp_i, ...
            hp_title_for_log, 'Error', message);
        write_run_issue_log(run_issue_log_filename, run_issue_log);
...
if total_rendered_figure_count == 0
    warning(['No figures were rendered. Check hyperparameters, metadata index, ' ...
        'data files, and run_issue_log_category_pairwise.txt.']);
else
    progress_log('SCRIPT', 'Rendered %d figures across all hyperparameter sets.', ...
        total_rendered_figure_count);
end

if ~isempty(run_issue_log)
    write_run_issue_log(run_issue_log_filename, run_issue_log);
    progress_log('SCRIPT', 'Run issue log written to: %s', run_issue_log_filename);
end

progress_log('SCRIPT', ...
    ['Finished in %.1f s. Valid kernel-session records=%d, ' ...
     'failed records=%d, figures=%d.'], ...
    toc(run_tic), total_valid_session_count, total_failed_session_count, ...
    total_rendered_figure_count);

function figure_configs = build_figure_configs()
    k = 1; % Every listed Long* metadata model contains one kernel block.
    ...
    figure_configs(1).output_stub = ...
        'Figure3_pooled_kernels_open_vs_close';
    figure_configs(1).figure_title = 'Pooled kernels: Open vs Close';
    ...
    figure_configs(2).output_stub = ...
        'Figure3_pooled_kernels_pre_vs_post';
    figure_configs(2).figure_title = 'Pooled kernels: Pre vs Post';
...
function loaded_states = load_all_required_states(root, meta, kernel_indices, err_multi)
    loaded_states = struct();
    preposts = {'Pre', 'Post'};
    states = {'RestOpen', 'RestClose'};
    n_conditions = numel(preposts) * numel(states);
    condition_i = 0;

    progress_log('LOAD', 'Loading %d condition files once for %s; kernels=%s.', ...
        n_conditions, make_session_label(meta), mat2str(kernel_indices));
    ...
    for pp_i = 1:numel(preposts)
        for st_i = 1:numel(states)
            ...
            pp = preposts{pp_i};
            st = states{st_i};
            progress_log('LOAD', '[%d/%d] prepost=%s, state=%s.', ...
                condition_i, n_conditions, pp, st);

            condition_states = load_condition_all_kernels( ...
                root, meta, pp, st, kernel_indices, err_multi);
            for k_i = 1:numel(kernel_indices)
                k = kernel_indices(k_i);
                loaded_states.(state_key(pp, st, k)) = condition_states(k_i);
            end
        end
    end
end

function condition_states = load_condition_all_kernels( ...
    root, meta, prepost, state, kernel_indices, err_multi)
    ...
    required_raster_fields = {'meta', 'data'};
    if ~all(isfield(raster_data, required_raster_fields)) || ...
            ~isfield(raster_data.data, 'cell_area')
        error('Raster file is missing meta/data/cell_area: %s', raster_filename);
    end

    cell_area = raster_data.data.cell_area;
    filter1 = ismember(cell_area, {'ACC'});
    filter2 = ismember(cell_area, {'VLPFC'});
    cell_ids = extract_optional_cell_ids(raster_data);

    progress_log('LOAD-RASTER', ...
        'Loaded %s %s; N=%d, ACC=%d, VLPFC=%d.', ...
        prepost, state, numel(cell_area), sum(filter1), sum(filter2));

    meta.filename = generate_filename('GLM', meta);
    glm_filename = fullfile(root, 'Data', 'Working', 'GLM', meta.filename);
    progress_log('LOAD-GLM', 'Loading once for all kernels: %s', glm_filename);
    GLM_data = load(glm_filename);

    if ~isfield(GLM_data, 'meta') || ~isfield(GLM_data.meta, 'N') || ...
            ~isfield(GLM_data, 'data') || ...
            ~isfield(GLM_data.data, 'model_par') || ...
            ~isfield(GLM_data.data, 'model_err') || ...
            ~isfield(GLM_data.data.model_err, 'total')
        error('GLM file is missing required fields: %s', glm_filename);
    end

    N = GLM_data.meta.N;
    model_par = GLM_data.data.model_par;
    model_err = GLM_data.data.model_err.total;

    if numel(cell_area) ~= N
        error('Raster cell_area length (%d) does not match GLM N (%d): %s', ...
            numel(cell_area), N, glm_filename);
    end
    if size(model_par, 1) ~= N || size(model_err, 1) ~= N
        error('GLM row dimensions do not match N=%d: %s', N, glm_filename);
    end
    if ~isequal(size(model_par), size(model_err))
        error('model_par and model_err.total sizes differ: %s', glm_filename);
    end

    max_kernel = max(kernel_indices);
    required_last_col = 1 + N * max_kernel;
    if required_last_col > size(model_par, 2)
        error(['Kernel %d requires columns through %d, but model_par/model_err ' ...
            'only contain %d columns: %s'], max_kernel, required_last_col, ...
            size(model_par, 2), glm_filename);
    end
    ...
    for k_i = 1:numel(kernel_indices)
        ...
        J = model_par(:, col_start:col_end);
        err = model_err(:, col_start:col_end);
        finite_err = err(isfinite(err));
        if any(finite_err < 0)
            error('Negative finite error values found for kernel %d: %s', ...
                kernel_idx, glm_filename);
        end
        ...
        state_struct.cell_ids = cell_ids;
        state_struct.filter1 = filter1;
        state_struct.filter2 = filter2;
        state_struct.cat12 = classify_connections( ...
            J(filter1, filter2), err(filter1, filter2), err_multi);
        state_struct.cat21 = classify_connections( ...
            J(filter2, filter1), err(filter2, filter1), err_multi);
        if k_i == 1
            condition_states = state_struct;
        else
            condition_states(k_i) = state_struct;
        end
    end

    progress_log('LOAD-GLM', ...
        'Loaded/classified %s %s for %d kernels in %.1f s.', ...
        prepost, state, numel(kernel_indices), toc(condition_tic));
end

function cell_ids = extract_optional_cell_ids(raster_data)
    cell_ids = [];
    candidate_fields = {'cell_id', 'cell_ids', 'unit_id', 'unit_ids', ...
        'cell_name', 'cell_names'};
    for i = 1:numel(candidate_fields)
        field = candidate_fields{i};
        if isfield(raster_data.data, field)
            candidate = raster_data.data.(field);
            if numel(candidate) == numel(raster_data.data.cell_area)
                cell_ids = candidate;
                return;
            end
        end
...
function text_out = condition_to_string(cond)
    text_out = sprintf('%s/%s', cond.prepost, cond.state);
end

function render_comparison_figure(cfg, pooled_one, params)
    render_tic = tic;
    f = figure('Color', 'w', 'Visible', params.figure_visible);
    figure_cleanup = onCleanup(@() close_figure_if_valid(f)); %#ok<NASGU>

    tiledlayout(params.n_row, params.n_col, ...
        'TileSpacing', 'Compact', 'Padding', 'Compact');
    tile_idx = @(row, col) rowcol_to_panel_index(row, col, params.n_col);

    plot_modes = {'raw', 'row', 'column', 'difference', ...
        'normalized_difference', 'oe_ratio', 'standardized_residual'};
    column_titles = {'raw count', 'normalized by row', 'normalized by column', ...
        'difference', 'normalized difference', 'O/E ratio', 'Pearson residual'};
    colorbar_labels = {'count', 'row fraction', 'column fraction', ...
        'count difference', 'normalized difference', 'observed / expected', ...
        '(O - E) / sqrt(E)'};
    value_modes = {'integer', 'fraction', 'fraction', ...
        'signed', 'signed', 'ratio', 'signed'};
    color_modes = {'sequential', 'sequential', 'sequential', ...
        'signed', 'normalized_signed', 'sequential', 'signed'};

    n_panels = params.n_row * params.n_col;
    progress_log('RENDER', 'Building %d panels for %s.', n_panels, cfg.output_stub);

    for ctx_i = 1:numel(cfg.contexts)
        ctx = cfg.contexts(ctx_i);
        row_top = (ctx_i - 1) * 2 + 1;
        row_bottom = row_top + 1;
        counts3 = pooled_one.contexts(ctx_i).cat_counts;
        counts2 = counts3([1, 3], [1, 3]);

        [agreement3, kappa3, n3] = summarize_counts(counts3);
        [agreement2, kappa2, n2] = summarize_counts(counts2);

        for col_i = 1:numel(plot_modes)
            mode = plot_modes{col_i};

            mat3 = transform_category_matrix(counts3, mode);
            ax = nexttile(tile_idx(row_top, col_i));
            plot_transition_table_generic(ax, mat3, params.category_labels, ...
                agreement3, kappa3, n3, sprintf('%s categorical 3x3', ctx.name), ...
                sprintf('%s category', ctx.x.axis_label), ...
                sprintf('%s category', ctx.y.axis_label), ...
                column_titles{col_i}, colorbar_labels{col_i}, ...
                value_modes{col_i}, color_modes{col_i}, params.diverging_cmap);
            add_panel_label(ax, row_top, col_i, params.n_col);

            mat2 = transform_category_matrix(counts2, mode);
            ax = nexttile(tile_idx(row_bottom, col_i));
            plot_transition_table_generic(ax, mat2, params.category_labels_2x2, ...
                agreement2, kappa2, n2, sprintf('%s categorical 2x2', ctx.name), ...
                sprintf('%s category', ctx.x.axis_label), ...
                sprintf('%s category', ctx.y.axis_label), ...
                column_titles{col_i}, colorbar_labels{col_i}, ...
                value_modes{col_i}, color_modes{col_i}, params.diverging_cmap);
            add_panel_label(ax, row_bottom, col_i, params.n_col);
        end
    end

    sgtitle(sprintf('Supplement Fig 3: %s\n%s; valid kernel-session records = %d', ...
        cfg.figure_title, params.hyperparam_title, ...
        pooled_one.valid_session_count), 'Interpreter', 'none');
    ...
    if params.export_jpg
        jpg_filename = fullfile(params.output_folder, ...
            [cfg.output_stub, '_supp_preview.jpg']);
    ...
    if params.export_pdf
        pdf_filename = fullfile(params.output_folder, ...
            [cfg.output_stub, '_supp.pdf']);
        progress_log('EXPORT', 'Writing PDF: %s', pdf_filename);
        exportgraphics(f, pdf_filename, 'ContentType', 'vector', ...
            'BackgroundColor', 'white');
    end

    if ~params.export_jpg && ~params.export_pdf
        progress_log('EXPORT', 'Both export switches are disabled; figure not written.');
    end

    progress_log('RENDER', 'Completed %s in %.1f s.', ...
        cfg.output_stub, toc(render_tic));
end
...
function mat_out = transform_category_matrix(counts, mode)
    ...
    switch lower(mode)
        ...
        case 'row'
            ...
            valid = denom > 0;
            mat_out(valid, :) = counts(valid, :) ./ denom(valid);
        ...
        case 'oe_ratio'
            ...
            if total_n > 0
                ...
                row_prob = sum(counts, 2) / total_n;
                col_prob = sum(counts, 1) / total_n;
                expected_prob = row_prob * col_prob;
        ...
        case 'standardized_residual'
            mat_out = nan(size(counts));
            if total_n > 0
                expected_counts = sum(counts, 2) * sum(counts, 1) / total_n;
                valid = expected_counts > 0;
                mat_out(valid) = (counts(valid) - expected_counts(valid)) ./ ...
                    sqrt(expected_counts(valid));
            end

        otherwise
            error('Unknown matrix transform mode: %s', mode);
    end
end

function plot_transition_table_generic(ax, mat, category_labels, ...
    agreement, kappa, n_valid, title_text, xlabel_text, ylabel_text, ...
    normalization_label, colorbar_label, value_mode, color_mode, diverging_cmap)
    ...
    switch color_mode
        ...
        otherwise
            ...
            if isempty(finite_vals)
                caxis(ax, [0, 1]);
            elseif all(finite_vals == finite_vals(1))
                delta = max(1, abs(finite_vals(1))) * 0.01;
                caxis(ax, [finite_vals(1) - delta, finite_vals(1) + delta]);
            end
    end
    ...
    for y_idx = 1:n_cat
        for x_idx = 1:n_cat
            ...
            if ~isfinite(this_val)
                ...
                text_color = contrasting_text_color(this_val, limits, cmap);
            end
            ...
            text(ax, x_idx, y_idx, value_str, ...
                ...
                'FontWeight', 'bold', ...
                'Color', text_color, ...
                'FontSize', choose_cell_font_size(n_cat));
        end
    end
    ...
    title(ax, sprintf('%s\n%s\nAgreement = %s, kappa = %s, n = %d', ...
        title_text, normalization_label, agreement_str, kappa_str, n_valid), ...
        'Interpreter', 'none');
end

function text_color = contrasting_text_color(value, limits, cmap)
    if limits(2) <= limits(1) || isempty(cmap)
        text_color = 'k';
        return;
    end
    ratio = (value - limits(1)) / (limits(2) - limits(1));
    ratio = min(max(ratio, 0), 1);
    idx = 1 + round(ratio * (size(cmap, 1) - 1));
    rgb = cmap(idx, :);
...
function counts = make_pair_category_counts(state_x, state_y)
    validate_matching_filters(state_x, state_y);

    x_cat = [state_x.cat12(:); state_x.cat21(:)];
    y_cat = [state_y.cat12(:); state_y.cat21(:)];
    valid = isfinite(x_cat) & isfinite(y_cat);
    x_code = double(x_cat(valid)) + 2;
    y_code = double(y_cat(valid)) + 2;

    if isempty(x_code)
        counts = zeros(3, 3);
    else
        counts = accumarray([x_code, y_code], 1, [3, 3], @sum, 0);
    end
end

function cat = classify_connections(J, err, err_multi)
    if ~isscalar(err_multi) || ~isfinite(err_multi) || err_multi < 0
        error('err_multi must be a finite nonnegative scalar.');
    end
    if ~isequal(size(J), size(err))
        error('J and err must have matching sizes.');
    end
...
function validate_matching_filters(state_a, state_b)
    if numel(state_a.filter1) ~= numel(state_b.filter1) || ...
            any(state_a.filter1(:) ~= state_b.filter1(:)) || ...
            numel(state_a.filter2) ~= numel(state_b.filter2) || ...
            any(state_a.filter2(:) ~= state_b.filter2(:))
        error('State area filters do not match between comparison members.');
    end

    if ~isequal(size(state_a.cat12), size(state_b.cat12)) || ...
            ~isequal(size(state_a.cat21), size(state_b.cat21))
        error('State category matrices do not have matching sizes.');
    end

    if ~isempty(state_a.cell_ids) && ~isempty(state_b.cell_ids) && ...
            ~isequal(string(state_a.cell_ids(:)), string(state_b.cell_ids(:)))
        error('Cell identities or ordering do not match between comparison members.');
    end
end
