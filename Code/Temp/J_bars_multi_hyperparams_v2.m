    ...
    for injection_idx = 1:numel(injection_types)
        ...
        for state_idx = 1:numel(states)
            ...
            for posneg_idx = 1:2
                ...
                ratios_plot = reshape( ...
                    ratios_all(:, posneg_idx, :), ...
                    n_kernel, numel(prepost_labels));
                low_errs_plot = reshape( ...
                    low_errs_all(:, posneg_idx, :), ...
                    n_kernel, numel(prepost_labels));
                high_errs_plot = reshape( ...
                    high_errs_all(:, posneg_idx, :), ...
                    n_kernel, numel(prepost_labels));

                plot_kernel_grouped_bars( ...
                    tile, ratios_plot, low_errs_plot, high_errs_plot, ...
...
function [ratios, low_errs, high_errs, sig_labels] = compute_bar_stats(J_counts, total_counts)
    ...
    low_errs = ratios - CIs(:, :, 1) * 100;
    high_errs = CIs(:, :, 2) * 100 - ratios;
end
...
% Dimension policy:
%   Avoid squeeze because kernel_num can equal 1. All plotting slices are
%   explicitly reshaped to [kernel x condition] or [kernel x 1].
...
function plot_effect_metric_combined_pooled_ext(root, stats_grid, states, state_labels, posneg_labels, ...
    ...
    for state_idx = 1:numel(states)
        for posneg_idx = 1:2
            ...
            values_plot = reshape( ...
                values(state_idx, posneg_idx, :, :), ...
                numel(kernel_indices), numel(injection_types));
            low_errs_plot = reshape( ...
                low_errs(state_idx, posneg_idx, :, :), ...
                numel(kernel_indices), numel(injection_types));
            high_errs_plot = reshape( ...
                high_errs(state_idx, posneg_idx, :, :), ...
                numel(kernel_indices), numel(injection_types));
            bar_sig_labels_plot = reshape( ...
                prepost_sig_labels(state_idx, posneg_idx, :, :), ...
                numel(kernel_indices), numel(injection_types));
            did_sig_labels_plot = reshape( ...
                did_sig_labels(state_idx, posneg_idx, :), ...
                numel(kernel_indices), 1);

            plot_metric_grouped_bars_ext( ...
                tile, values_plot, low_errs_plot, high_errs_plot, ...
                bar_sig_labels_plot, did_sig_labels_plot, ...
...
function plot_effect_metric_single_injection_pooled_ext(root, stats_grid, states, state_labels, posneg_labels, ...
    ...
    for state_idx = 1:numel(states)
        for posneg_idx = 1:2
            ...
            values_plot = reshape( ...
                values_one(state_idx, posneg_idx, :), ...
                numel(kernel_indices), 1);
            low_errs_plot = reshape( ...
                low_one(state_idx, posneg_idx, :), ...
                numel(kernel_indices), 1);
            high_errs_plot = reshape( ...
                high_one(state_idx, posneg_idx, :), ...
                numel(kernel_indices), 1);
            labels_plot = reshape( ...
                labels_one(state_idx, posneg_idx, :), ...
                numel(kernel_indices), 1);

            plot_metric_single_bars_ext( ...
                tile, values_plot, low_errs_plot, high_errs_plot, ...
                labels_plot, kernel_labels, y_limit, metric_mode, ...
                injection_label);
            ylabel(tile, metric_axis_label_ext(metric_mode));
            title(tile, sprintf('%s, %s', state_labels{state_idx}, posneg_labels{posneg_idx}));
        end
...
function plot_prepost_percent_injection_pooled_ext(root, stats_grid, states, state_labels, posneg_labels, ...
    ...
    for state_idx = 1:n_state
        for posneg_idx = 1:n_sign
            ...
            ratios_plot = reshape( ...
                ratios(state_idx, posneg_idx, :, :), ...
                numel(kernel_indices), 2);
            low_errs_plot = reshape( ...
                low_errs(state_idx, posneg_idx, :, :), ...
                numel(kernel_indices), 2);
            high_errs_plot = reshape( ...
                high_errs(state_idx, posneg_idx, :, :), ...
                numel(kernel_indices), 2);
            sig_labels_plot = reshape( ...
                sig_labels(state_idx, posneg_idx, :), ...
                numel(kernel_indices), 1);

            plot_prepost_grouped_bars_ext( ...
                tile, ratios_plot, low_errs_plot, high_errs_plot, ...
                sig_labels_plot, {'Pre', 'Post'}, kernel_labels, ...
                bar_offset, bar_width, y_limit);
            ylabel(tile, 'Significant J %');
            title(tile, sprintf('%s, %s', state_labels{state_idx}, posneg_labels{posneg_idx}));
        end
...
function plot_muscimol_minus_saline_effect_pooled_ext(root, stats_grid, states, state_labels, posneg_labels, ...
    ...
    for state_idx = 1:numel(states)
        for posneg_idx = 1:2
            ...
            did_values_plot = reshape( ...
                did_values(state_idx, posneg_idx, :), ...
                numel(kernel_indices), 1);
            did_low_errs_plot = reshape( ...
                did_low_errs(state_idx, posneg_idx, :), ...
                numel(kernel_indices), 1);
            did_high_errs_plot = reshape( ...
                did_high_errs(state_idx, posneg_idx, :), ...
                numel(kernel_indices), 1);
            did_sig_labels_plot = reshape( ...
                did_sig_labels(state_idx, posneg_idx, :), ...
                numel(kernel_indices), 1);
            ...
            plot_metric_single_bars_ext( ...
                tile, did_values_plot, did_low_errs_plot, ...
                did_high_errs_plot, did_sig_labels_plot, ...
                kernel_labels, y_limit, metric_mode, 'DID');
