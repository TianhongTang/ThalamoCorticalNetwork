%% adaptation.m - Temporary script for adapting data across versions
...
for i = 1:numel(data_folders)
    ...
    % folder filter
    if ~strcmp(data_folder_name, 'raster')
        continue; 
    end
    ...
    for file_idx = 1:numel(all_files)
        ...
        if ~isfield(meta, 'trial_len') || ~isfield(meta, 'max_len') || ~isfield(meta, 'min_len') || ~isfield(meta, 'total_len')
            meta.trial_len = sort(data.trial_len, 'descend');
            meta.max_len   = max(data.trial_len);
            meta.min_len   = min(data.trial_len);
            meta.total_len = sum(data.trial_len);
            % Update the mat file with new meta and data
