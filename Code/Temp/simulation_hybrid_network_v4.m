%% simulation_hybrid_network_v4.m
...
% Temporal kernels are always loaded from the external project kernel file,
% including when model_size_source='fixed'.
...
% kernel selected by config.reference.kernel_name.
% -------------------------------------------------------------------------
...
function validate_simulation_config(config)
    ...
    if strlength(string(config.reference.kernel_name)) == 0
        error(['config.reference.kernel_name must identify an external ' ...
               'kernel file in every model-size mode.']);
    end
    if strcmp(config.model_size_source, 'fixed')
        fixed_counts = [ ...
            config.fixed_model.cell_counts.ACC, ...
...
function [sim_meta, network, kernels, source_info] = ...
    ...
    source_info = struct();
    % External-reference flags distinguish experimental raster/GLM data
    % from the temporal-kernel definition, which is external in both modes.
...
function cell_info = build_cell_layout(config, cortical_source, raster_source)
    ...
    cell_area(idx_thal_acc) = "Thalamus";
    cell_area(idx_thal_vlpfc) = "Thalamus";
...
function name = resolve_effective_kernel_name(config)
    % The temporal kernel is externally selected in both size modes.
