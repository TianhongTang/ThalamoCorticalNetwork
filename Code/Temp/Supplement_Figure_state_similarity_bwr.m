...
% Each panel uses a symmetric blue-white-red color scale:
%   #0000FF at the negative limit, #FFFFFF at zero, and #FF0000 at
%   the positive limit. The limits are based on the maximum absolute
%   finite off-diagonal similarity value.
...
function plot_similarity_heatmap( ...
    ...
    % Determine a symmetric color range from finite off-diagonal values.
    % Zero is therefore always located exactly at the white midpoint.
    ...
    color_limits = calculate_symmetric_color_limits( ...
        off_diagonal_values, [-1, 1]);

    % Use an odd number of colors so the exact center entry is white.
    diverging_colormap = blue_white_red_colormap(257);
    ...
    % Non-finite cells are displayed in light gray.
    ...
    % Explicit ticks make the zero-centered symmetry visually clear.
    color_max = color_limits(2);
    cb.Ticks = [-color_max, 0, color_max];
    ...
    for row_idx = 1:n_state
        for col_idx = 1:n_state
            ...
            if ~isfinite(value)
                ...
                text_color = choose_heatmap_text_color( ...
                    value, color_limits, diverging_colormap);
            end
...
function color_limits = calculate_symmetric_color_limits( ...
    ...
    if lower_bound >= 0 || upper_bound <= 0
        error('metric_bounds must span zero.');
    end

    maximum_allowed_abs = min(abs([lower_bound, upper_bound]));

    if isempty(values)
        max_abs_value = maximum_allowed_abs;
    else
        max_abs_value = max(abs(values));

        if ~isfinite(max_abs_value)
            max_abs_value = maximum_allowed_abs;
        end
    end

    % caxis requires distinct limits. If every finite off-diagonal value
    % is zero, use a narrow symmetric range centered on zero.
    if max_abs_value == 0
        max_abs_value = min(0.05, maximum_allowed_abs);
    end

    max_abs_value = min(max_abs_value, maximum_allowed_abs);
    color_limits = [-max_abs_value, max_abs_value];
end


function cmap = blue_white_red_colormap(n_colors)
    ...
    if n_colors < 3 || n_colors ~= floor(n_colors)
        error('n_colors must be an integer of at least 3.');
    end

    % Force an odd number so one entry is exactly white.
    if mod(n_colors, 2) == 0
        n_colors = n_colors + 1;
    end

    half_count = (n_colors + 1) / 2;

    blue_to_white = [ ...
        linspace(0, 1, half_count)', ...
        linspace(0, 1, half_count)', ...
        ones(half_count, 1)];

    white_to_red = [ ...
        ones(half_count, 1), ...
        linspace(1, 0, half_count)', ...
        linspace(1, 0, half_count)'];

    % Remove the duplicated white row from the second half.
    cmap = [blue_to_white; white_to_red(2:end, :)];
end


function text_color = choose_heatmap_text_color( ...
    value, color_limits, cmap)
    ...
    color_idx = 1 + round(normalized_value * (size(cmap, 1) - 1));
    cell_color = cmap(color_idx, :);

    % Relative luminance approximation. White text is used only on
    % sufficiently dark blue/red backgrounds.
    luminance = ...
        0.2126 * cell_color(1) + ...
        0.7152 * cell_color(2) + ...
        0.0722 * cell_color(3);
