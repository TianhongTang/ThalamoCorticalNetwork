...
base_plot_params.n_row = 2;
base_plot_params.n_col = 7;
base_plot_params.category_labels_9 = {'++', '+o', '+-', 'o+', 'oo', 'o-', '-+', '-o', '--'};
base_plot_params.category_labels_4 = {'++', '+-', '-+', '--'};

cache_root = fullfile(root, 'Data', 'Working', 'FigureCache', 'SuppFig_OC_PrePost_pattern_transition');
output_root = fullfile(root, 'Figures/Paper/Fig3_Supp');
check_path(output_root);
run_issue_log = empty_run_issue_log();
run_issue_log_filename = fullfile(output_root, 'run_issue_log_plot_from_precomputed.txt');

for hp_i = 1:numel(hyperparam_configs)
    try
        hp = fill_default_hyperparams(hyperparam_configs(hp_i));
        hp_title = make_hyperparam_title(hp);
        cache_file = fullfile(cache_root, make_hyperparam_folder_name(hp), 'precomputed_compound_pattern.mat');
        if ~exist(cache_file, 'file')
            message = sprintf('Missing precomputed MAT: %s', cache_file);
            warning('%s', message);
            run_issue_log = append_run_issue(run_issue_log, hp_i, hp_title, 'MissingPrecomputed', message);
        ...
        params.category_labels_9 = base_plot_params.category_labels_9;
        params.category_labels_4 = base_plot_params.category_labels_4;
        params.output_folder = fullfile(output_root, make_hyperparam_folder_name(precomputed.hp));
        params.hyperparam_title = hp_title;
        check_path(params.output_folder);

        figure_configs = precomputed.figure_configs;
        pooled = precomputed.pooled;

        fprintf('\n===== Plotting %d/%d: %s =====\n', hp_i, numel(hyperparam_configs), hp_title);
        ...
        write_run_issue_log(run_issue_log_filename, run_issue_log);
        warning('Skipping plot for hyperparameter configuration %d after error: %s', hp_i, ME.message);
        continue;
    end
end
