...
%% ----------------Exp pure kernels: Exp5 / Exp40 / Exp100 / Exp40mix / Exp100mix
% These are connection-only kernels with no post-spike kernels.
% Exp5 is intentionally identical in structure to SinglePure:
%   T = 200 ms, k(t) = exp(-t/tau), normalized to sum 1.
% Mixed kernels contain multiple exponential basis kernels of the same length.
...
exp_kernel_specs = {
    'Exp5',      [5],          {'Exponential 5ms'};
    'Exp40',     [40],         {'Exponential 40ms'};
    'Exp100',    [100],        {'Exponential 100ms'};
    'Exp40mix',  [5, 40],      {'Exponential 5ms', 'Exponential 40ms'};
    'Exp100mix', [5, 40, 100], {'Exponential 5ms', 'Exponential 40ms', 'Exponential 100ms'};
};

for spec_idx = 1:size(exp_kernel_specs, 1)
    kernel_name = exp_kernel_specs{spec_idx, 1};
    taus = exp_kernel_specs{spec_idx, 2};
    legend_names = exp_kernel_specs{spec_idx, 3};

    conn_kernels = cell(1, numel(taus));
    for k_idx = 1:numel(taus)
        tau = taus(k_idx);
        kernel = exp(-t ./ tau);
        kernel = kernel / sum(kernel);
        conn_kernels{k_idx} = kernel;
    end
    ...
    figure("Visible", "off");
    hold on;
    for k_idx = 1:n_conn_kernel
        plot(t, conn_kernels{k_idx}, 'LineWidth', 1);
    end
    hold off;
    xlabel('Time (ms)');
    ylabel('Amplitude');
    title(['Kernel: ', kernel_name]);
    legend(legend_names, 'Location', 'northeast');
    saveas(gcf, fullfile(save_folder, ['kernel_', kernel_name, '.png']));
    close(gcf);
