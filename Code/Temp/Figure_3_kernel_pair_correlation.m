%% Figure 3: Aggregated multi-session kernel-kernel correlation analysis
% Data from all selected metadata rows are pooled into one figure.
% This version compares kernel_idx_1 vs kernel_idx_2 within the same state.
% Rows are organized as two rows per state:
%   row 2*i-1: kernel 1 network, all-connection scatter, either-sig scatter, same-sign abs scatter, 3x3 category table.
%   row 2*i  : kernel 2 network, all-connection density, both-sig scatter, sign-switch abs scatter, 2x2 category table.
...
n_compare = numel(states);

%% Parameters

% Compare these two kernel columns from the same GLM result.
kernel_idx_1 = 1;
kernel_idx_2 = 2;
kernel_labels = {sprintf('K%d', kernel_idx_1), sprintf('K%d', kernel_idx_2)};
...
% from one GLM kernel configuration, then compares kernel columns inside
% that GLM result using kernel_idx_1 and kernel_idx_2.
% -------------------------------------------------------------------------
...
%% Pool kernel-kernel comparison data across all selected sessions
...
first_valid_kernel1_state_data = [];
first_valid_kernel2_state_data = [];
first_valid_label = '';
...
for session_i = 1:numel(meta_array)
    ...
    try
        kernel1_state_data = struct([]);
        kernel2_state_data = struct([]);

        for state_i = 1:n_compare
            sd1 = load_state_connectivity(root, meta, preposts{state_i}, states{state_i}, kernel_idx_1);
            sd2 = load_state_connectivity(root, meta, preposts{state_i}, states{state_i}, kernel_idx_2);

            if state_i == 1
                kernel1_state_data = sd1;
                kernel2_state_data = sd2;
            else
                kernel1_state_data = [kernel1_state_data; sd1]; %#ok<AGROW>
                kernel2_state_data = [kernel2_state_data; sd2]; %#ok<AGROW>
            end

            data_pair = make_kernel_pair_vectors(sd1, sd2, err_multi);
            data_all(state_i) = append_connection_data(data_all(state_i), data_pair);

            [cat_counts, ~, ~, ~] = make_kernel_pair_category_counts(sd1, sd2, err_multi);
        ...
        if isempty(first_valid_kernel1_state_data) || session_i == example_session_idx
            first_valid_kernel1_state_data = kernel1_state_data;
            first_valid_kernel2_state_data = kernel2_state_data;
...
for state_i = 1:n_compare
    ...
    fprintf('Pooled %s %s %s-vs-%s all signed: rho = %.6f, p = %.3e, n = %d\n', ...
        preposts{state_i}, states{state_i}, kernel_labels{1}, kernel_labels{2}, ...
        stats(state_i).rho_orig, stats(state_i).p_orig, stats(state_i).n_orig);
    fprintf('Pooled %s %s categorical agreement: agreement = %.6f, kappa = %.6f, n = %d\n', ...
        preposts{state_i}, states{state_i}, ...
...
x_name = kernel_labels{1};
y_name = kernel_labels{2};

for state_i = 1:n_compare
    row_top = 2 * state_i - 1;
    row_bottom = 2 * state_i;
    state_label = sprintf('%s %s', preposts{state_i}, states{state_i});
    title_prefix = sprintf('%s: %s vs %s', state_label, x_name, y_name);

    % Network examples: top row = kernel 1, bottom row = kernel 2.
    ax = nexttile(tile_idx(row_top, 1));
    call_plot_network(ax, ...
        first_valid_kernel1_state_data(state_i).J12, first_valid_kernel1_state_data(state_i).J21, ...
        first_valid_kernel1_state_data(state_i).err12, first_valid_kernel1_state_data(state_i).err21, ...
        network_err_multi, [], []);
    title(ax, sprintf('Example network: %s\n%s, %s', ...
        first_valid_label, state_label, x_name), 'Interpreter', 'none');

    ax = nexttile(tile_idx(row_bottom, 1));
    call_plot_network(ax, ...
        first_valid_kernel2_state_data(state_i).J12, first_valid_kernel2_state_data(state_i).J21, ...
        first_valid_kernel2_state_data(state_i).err12, first_valid_kernel2_state_data(state_i).err21, ...
        network_err_multi, [], []);
    title(ax, sprintf('Example network: %s\n%s, %s', ...
        first_valid_label, state_label, y_name), 'Interpreter', 'none');

    % All-connection signed scatter and density.
    ax = nexttile(tile_idx(row_top, 2));
    plot_kernel_pair_scatter(ax, data_all(state_i), 'all_signed', ...
    ...
    plot_kernel_pair_density(ax, data_all(state_i).x_orig, data_all(state_i).y_orig, ...
        stats(state_i).rho_orig, stats(state_i).p_orig, stats(state_i).n_orig, ...
    ...
    plot_kernel_pair_scatter(ax, data_all(state_i), 'either_sig_signed', ...
        scatter_marker_size, scatter_alpha, colors, show_legend, ...
    ...
    plot_kernel_pair_scatter(ax, data_all(state_i), 'both_sig_signed', ...
        scatter_marker_size, scatter_alpha, colors, show_legend, ...
...
filename = fullfile(save_folder, sprintf('Figure3_kernel%d_vs_kernel%d_preview.jpg', kernel_idx_1, kernel_idx_2));
exportgraphics(fig, filename, ...
    'ContentType', 'image', ...
    'BackgroundColor', 'white', ...
    'Resolution', resolution);
filename = fullfile(save_folder, sprintf('Figure3_kernel%d_vs_kernel%d.pdf', kernel_idx_1, kernel_idx_2));
exportgraphics(fig, filename, ...
    'ContentType', 'vector', ...
    'BackgroundColor', 'white', ...
    'Resolution', resolution);
...
function [data, label_text] = make_kernel_pair_vectors(kernel1_state, kernel2_state, err_multi)
    validate_matching_filters(kernel1_state, kernel2_state);

    x12 = kernel1_state.J12(:);
    y12 = kernel2_state.J12(:);
    x21 = kernel1_state.J21(:);
    y21 = kernel2_state.J21(:);
    xerr12 = kernel1_state.err12(:);
    yerr12 = kernel2_state.err12(:);
    xerr21 = kernel1_state.err21(:);
    yerr21 = kernel2_state.err21(:);
...
function [counts, agreement, kappa, n_valid] = make_kernel_pair_category_counts(kernel1_state, kernel2_state, err_multi)
    validate_matching_filters(kernel1_state, kernel2_state);

    x_cat12 = classify_connections(kernel1_state.J12(:), kernel1_state.err12(:), err_multi);
    y_cat12 = classify_connections(kernel2_state.J12(:), kernel2_state.err12(:), err_multi);
    x_cat21 = classify_connections(kernel1_state.J21(:), kernel1_state.err21(:), err_multi);
    y_cat21 = classify_connections(kernel2_state.J21(:), kernel2_state.err21(:), err_multi);
...
function plot_kernel_pair_scatter(ax, data, plot_mode, marker_size, marker_alpha, colors, show_legend, title_text, x_name, y_name)
    cla(ax);
    hold(ax, 'on');
...
function plot_kernel_pair_density(ax, x, y, rho, pval, n_valid, nbin, clip_percentile, use_log_count, title_text, axis_mode, colors, x_name, y_name)
