%% Figure 3: Combined pooled comparison script (6 figure versions)
% This script generates 6 separate 4x5 figures and saves each as PDF + JPG.
% Figure versions:
...
%   Row 1 = context A, first member (network + scatter/category panels)
%   Row 2 = context A, second member (network + density/category panels)
%   Row 3 = context B, first member
%   Row 4 = context B, second member
...
function render_comparison_figure(root, cfg, pooled_one, params)
    ...
    tiles = tiledlayout(params.n_row, params.n_col, 'TileSpacing', 'Compact', 'Padding', 'Compact'); %#ok<NASGU>
    tile_idx = @(row, col) (row - 1) * params.n_col + col;

    for ctx_i = 1:numel(cfg.contexts)
        ctx = cfg.contexts(ctx_i);
        row_top = (ctx_i - 1) * 2 + 1;
        row_bottom = row_top + 1;
        ex_x = pooled_one.example_states{row_top};
        ex_y = pooled_one.example_states{row_bottom};
        data = pooled_one.contexts(ctx_i).data;
        counts = pooled_one.contexts(ctx_i).cat_counts;

        [rho_orig, p_orig, n_orig] = pearson_stats(data.x_orig, data.y_orig);
        ...
        call_plot_network(ax, ex_x.J12, ex_x.J21, ex_x.err12, ex_x.err21, params.network_err_multi, [], []);
        title(ax, sprintf('Example network: %s\n%s', make_condition_title(ctx.x), pooled_one.example_label), 'Interpreter', 'none');

        ax = nexttile(tile_idx(row_bottom, 1));
        call_plot_network(ax, ex_y.J12, ex_y.J21, ex_y.err12, ex_y.err21, params.network_err_multi, [], []);
        title(ax, sprintf('Example network: %s\n%s', make_condition_title(ctx.y), pooled_one.example_label), 'Interpreter', 'none');
        ...
        plot_pair_scatter(ax, data, 'all_signed', params.scatter_marker_size, params.scatter_alpha, params.colors, params.show_legend, ...
            sprintf('%s all connections, sessions=%d', ctx.name, pooled_one.valid_session_count), ctx.x.axis_label, ctx.y.axis_label);

        ax = nexttile(tile_idx(row_bottom, 2));
        plot_pair_density(ax, data.x_orig, data.y_orig, rho_orig, p_orig, n_orig, params.density_nbin, params.density_clip_percentile, ...
            params.density_use_log_count, sprintf('%s all-connection density, sessions=%d', ctx.name, pooled_one.valid_session_count), ...
            'signed', params.colors, ctx.x.axis_label, ctx.y.axis_label);
...
function plot_pair_scatter(ax, data, plot_mode, marker_size, marker_alpha, colors, show_legend, title_text, x_label, y_label)
    cla(ax);
    hold(ax, 'on');
...
function plot_pair_density(ax, x, y, rho, pval, n_valid, nbin, clip_percentile, use_log_count, title_text, axis_mode, colors, x_label, y_label)
