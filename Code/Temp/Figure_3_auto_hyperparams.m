...
% Batch version with automatically generated hyperparameter configurations.
%
% For each hyperparameter configuration, this script generates:
%   1. Open vs Close figures for each kernel index, split by Pre/Post.
%   2. Pre vs Post figures for each kernel index, split by RestOpen/RestClose.
%   3. Kernel-vs-kernel figures for every kernel pair, split by RestOpen/RestClose.
%
% The number of saved figures is dynamic:
%   n_fig = 2 * kernel_num + 2 * nchoosek(kernel_num, 2).
%
% Outputs are saved under:
%   Figures/Paper/fig3/<hyperparameter-folder>/
...
params.preferred_example_session_index = preferred_example_session_index;
params.skip_failed_sessions = skip_failed_sessions;
params.max_sessions_to_include = max_sessions_to_include;
params.colors = colors;

%% Hyperparameter configurations
% Automatically generate:
%   42 kernel settings x 2 injections x 2 alignments = 168 configurations.
% Fixed:
%   reg_name = "L2=0_2";
...
%% Run all hyperparameter configurations
base_output_folder = fullfile(root, 'Figures', 'Paper', 'fig3');
check_path(base_output_folder);
log_filename = fullfile(base_output_folder, 'run_issue_log_figure3.txt');
...
for hp_i = 1:numel(hyperparam_configs)
    ...
    try
        run_one_hyperparam_config(root, mt, hp, params, base_output_folder);
    catch ME
        log_hyperparam_issue(log_filename, hp_i, numel(hyperparam_configs), hp, ME);
        warning('Skipping hyperparameter configuration %d/%d after error: %s', ...
            hp_i, numel(hyperparam_configs), ME.message);
        continue;
    end
end
