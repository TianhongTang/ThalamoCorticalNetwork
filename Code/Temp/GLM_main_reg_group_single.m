%% GLM_main_reg_group_single.m - GLM fitting for one selected registered task group.
% Select exactly one task group using one of:
%   GLM_TASK_ID=1..11
%   GLM_TRAINING_TASK=<group name>
%   GLM_TASK_GROUP=<group name>
%
% Task IDs:
%   1  = LongExpParams
%   2  = LongExpMix
...
%
% Intended Slurm use:
%   sbatch cluster_gpu_reg_single.sh 10
...
function training_task = resolve_single_training_task(available_training_tasks)
    ...
    if isempty(env_idx)
        error(['No task group selected. Set GLM_TASK_ID=1..%d or GLM_TRAINING_TASK=<name>. ', ...
               'For Slurm, use: sbatch cluster_gpu_reg_single.sh 10'], numel(available_training_tasks));
    end
