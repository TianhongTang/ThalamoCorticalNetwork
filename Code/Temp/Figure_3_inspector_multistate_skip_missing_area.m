...
% Version: skip-missing-area sessions are warned and skipped, not fatal.
%
% Main outputs:
...
err_multi = 1;
% Missing-area sessions are skipped automatically because there is no
% well-defined corresponding connection set for the requested area pair.
skip_missing_area_sessions = true;
...
for session_i = 1:numel(meta_array)
    ...
    try
        ...
        if skip_missing_area_sessions && is_missing_area_error(ME)
            skipped_missing_area_count = skipped_missing_area_count + 1;
            warning('Skipping %s because a requested area is missing: %s', session_label, ME.message);
            continue;
        end
...
fprintf('\nLoaded sessions: %d. Failed sessions: %d. Skipped missing-area sessions: %d.\n', ...
    valid_session_count, failed_session_count, skipped_missing_area_count);
...
assignin('base', 'valid_session_count', valid_session_count);
assignin('base', 'failed_session_count', failed_session_count);
assignin('base', 'skipped_missing_area_count', skipped_missing_area_count);
...
function state_struct = load_state_connectivity(root, meta, prepost, state, kernel_idx, area1_names, area2_names, area1_label, area2_label)
    ...
    if ~any(filter1)
        error('Figure3Inspector:MissingArea', ...
            'No cells found for area1 (%s) in %s %s %s.', ...
            join_area_names(area1_names), meta.prepost, meta.state, meta.area);
    end
    if ~any(filter2)
        error('Figure3Inspector:MissingArea', ...
            'No cells found for area2 (%s) in %s %s %s.', ...
            join_area_names(area2_names), meta.prepost, meta.state, meta.area);
    end
...
function tf = is_missing_area_error(ME)
    tf = strcmp(ME.identifier, 'Figure3Inspector:MissingArea');
    if tf
        return;
    end

    for c = 1:numel(ME.cause)
        if is_missing_area_error(ME.cause{c})
            tf = true;
            return;
        end
    end
