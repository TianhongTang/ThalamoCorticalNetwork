...
for row_i = 1:n_panel_row
    for col_i = 1:n_panel_col
        ...
        plot_pair_scatter(ax, cfg.data, single_scatter_plot_mode, scatter_marker_size, scatter_alpha, ...
            ...
            axis_limit_override, show_inset_stat, show_identity_line, show_fit_line);
...
function plot_pair_scatter(ax, data, plot_mode, marker_size, marker_alpha, colors, show_legend, title_text, x_label, y_label, axis_limit_override, show_inset_stat, show_identity_line, show_fit_line)
    cla(ax);
    hold(ax, 'on');
    ...
    if show_fit_line
        plot_linear_fit_line(ax, x, y, axis_limit, colors.fit_line);
    end
...
function plot_linear_fit_line(ax, x, y, axis_limit, line_color)
    valid = isfinite(x) & isfinite(y);
    x = x(valid);
    y = y(valid);

    if numel(x) < 2 || all(x == x(1))
        return;
    end
