%% J_bidirectional_scatter.m - Reciprocal directional J scatter plots.
...
if by_session
    render_by_session_figures(root, meta_array, area_pairs, conditions, kernel_indices, ...
        ...
        combine_kernels, J_lim, lim_ratio, export_pdf, panel_label_pos);
else
    render_pooled_figures(root, meta_array, area_pairs, conditions, kernel_indices, ...
        matrix_direction, err_multi, colors, scatter_marker_size, scatter_alpha, figure_visible, ...
        show_legend, show_identity_line, show_fit_line, fit_line_method, show_histogram, ...
        combine_kernels, export_pdf, panel_label_pos);
end
...
function render_pooled_figures(root, meta_array, area_pairs, conditions, kernel_indices, ...
    matrix_direction, err_multi, colors, scatter_marker_size, scatter_alpha, figure_visible, ...
    show_legend, show_identity_line, show_fit_line, fit_line_method, show_histogram, combine_kernels, export_pdf, panel_label_pos)
    ...
    for pair_idx = 1:numel(area_pairs)
        ...
        if combine_kernels
            ...
            render_bidirectional_figure(root, pooled, valid_session_counts, area_pair, matrix_direction, ...
                ...
                show_legend, show_identity_line, show_fit_line, fit_line_method, show_histogram, ...
                NaN, NaN, '', false, 'J_bidirectional_scatter', export_pdf, panel_label_pos, render_count, render_total);
        else
            for kernel_i = 1:numel(kernel_indices)
                render_count = render_count + 1;
                ...
                render_bidirectional_figure(root, pooled(:, kernel_i), valid_session_counts(:, kernel_i), ...
                    ...
                    false, 'J_bidirectional_scatter', export_pdf, panel_label_pos, render_count, render_total);
            end
        end
    end
end
...
function render_by_session_figures(root, meta_array, area_pairs, conditions, kernel_indices, ...
    matrix_direction, err_multi, colors, scatter_marker_size, scatter_alpha, figure_visible, ...
    show_legend, show_identity_line, show_fit_line, fit_line_method, show_histogram, combine_kernels, J_lim, lim_ratio, export_pdf, panel_label_pos)
        ...
        for pair_idx = 1:numel(area_pairs)
            ...
            if combine_kernels
                ...
                render_bidirectional_figure(root, pooled, valid_session_counts, area_pair, matrix_direction, ...
                    conditions, kernel_indices, colors, scatter_marker_size, scatter_alpha, figure_visible, ...
                    show_legend, show_identity_line, show_fit_line, fit_line_method, show_histogram, ...
                    J_lim, lim_ratio, session_suffix, true, 'J_bidirectional_scatter_by_session', export_pdf, panel_label_pos, render_count, render_total);
            else
                for kernel_i = 1:numel(kernel_indices)
                    render_count = render_count + 1;
                    if ~has_any_reciprocal_data(pooled(:, kernel_i))
                    ...
                    render_bidirectional_figure(root, pooled(:, kernel_i), valid_session_counts(:, kernel_i), ...
                        ...
                        true, 'J_bidirectional_scatter_by_session', export_pdf, panel_label_pos, render_count, render_total);
                end
            end
        end
    end
...
function render_bidirectional_figure(root, pooled, valid_session_counts, area_pair, matrix_direction, ...
    ...
    plot_J_matrix_panel, output_prefix, export_pdf, panel_label_pos, render_count, render_total)

    panel_modes = {'all_signed', 'either_sig_signed', 'both_sig_signed', 'same_sign_abs', 'category_3x3', 'sig_J_matrix'};
    panel_titles = {'All connections', 'Either significant', 'Both significant', 'Same-sign abs', '3x3 category', 'Significant-only J matrix'};
...
function plot_bidirectional_scatter(ax, data, plot_mode, colors, marker_size, marker_alpha, title_text, x_label, y_label, axis_limit, show_legend, show_identity_line, show_fit_line, fit_line_method)
    ...
    switch plot_mode
        ...
        case 'both_sig_signed'
            plot_x = x(both_sig);
            plot_y = y(both_sig);
            pos_mask = data.x_cat == 1 & data.y_cat == 1;
            neg_mask = data.x_cat == -1 & data.y_cat == -1;
            switch_mask = both_sig & ~(pos_mask | neg_mask);
        ...
        case 'same_sign_abs'
            pos_mask = data.x_cat == 1 & data.y_cat == 1;
            neg_mask = data.x_cat == -1 & data.y_cat == -1;
            plot_x = abs(x(pos_mask | neg_mask));
            plot_y = abs(y(pos_mask | neg_mask));
            axis_limit = [0, max(3.5, get_positive_axis_max(plot_x, plot_y))];
            axis_mode = 'positive';
            scatter(ax, abs(x(pos_mask)), abs(y(pos_mask)), marker_size, 'filled', ...
                'MarkerFaceColor', colors.pos, 'MarkerFaceAlpha', marker_alpha, 'MarkerEdgeAlpha', marker_alpha, ...
                'DisplayName', 'pos');
            scatter(ax, abs(x(neg_mask)), abs(y(neg_mask)), marker_size, 'filled', ...
                'MarkerFaceColor', colors.neg, 'MarkerFaceAlpha', marker_alpha, 'MarkerEdgeAlpha', marker_alpha, ...
                'DisplayName', 'neg');
...
function [plot_x, plot_y, axis_mode] = get_plot_values_for_mode(data, plot_mode)
    x = data.x;
    y = data.y;
    x_sig = data.x_cat ~= 0;
    y_sig = data.y_cat ~= 0;
    ...
    switch plot_mode
        ...
        case 'same_sign_abs'
            pos_mask = data.x_cat == 1 & data.y_cat == 1;
            neg_mask = data.x_cat == -1 & data.y_cat == -1;
            same_sign = pos_mask | neg_mask;
            plot_x = abs(x(same_sign));
            plot_y = abs(y(same_sign));
            axis_mode = 'positive';

        otherwise
            error('Unknown plot_mode for histogram: %s.', plot_mode);
    end
end
