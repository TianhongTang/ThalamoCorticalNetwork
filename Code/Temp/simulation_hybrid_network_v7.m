%% simulation_hybrid_network_v7.m
...
% -------------------------------------------------------------------------
% 'match_data' = use reference raster size/dt and optionally GLM values.
% 'fixed'      = use fixed cell counts/dt and load no reference raster/GLM.
% Both modes load the externally selected kernel file below.
config.model_size_source = 'fixed';
...
% Manually specify the kernel name, expected connection-kernel count, and
% the generated-network scale assigned to each connection kernel.
% These values are strictly validated against the imported kernel file.
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
% FIXED MODEL PRESET
% Used only when config.model_size_source = 'fixed'. This section controls
% model size and dt only. Temporal kernels still come from the external
% kernel selected by config.kernel.name.
% -------------------------------------------------------------------------
...
% MAIN MODE SWITCHES
% -------------------------------------------------------------------------
config.network_source = 'generate';       % 'load' or 'generate'
config.tc_source = 'generate';        % 'load_pre', 'generate', or 'none'
config.cell_count_source = 'manual';    % 'data' or 'manual'
...
% Canonical output area is always 'Thalamus'. Legacy split labels are
% accepted only while reading older source files.
...
config.simulation.dt = []; % [] = copy meta.dt; missing dt falls back below.
config.simulation.default_dt = 0.001; % Project-wide default: 1 ms.
...
% -------------------------------------------------------------------------
% OUTPUT IDENTITY
% Each condition gets a different session_idx. Full and Cortex views share
% that session index and are distinguished by meta.area.
% -------------------------------------------------------------------------
if strcmp(config.model_size_source, 'fixed')
    config.output.simulation_tag = sprintf( ...
        'fixed_ACC%d_VLPFC%d_ThalACC%d_ThalVLPFC%d_kernel_%s_%s', ...
...
function config = default_simulation_config()
    ...
    config.kernel = struct();
    config.kernel.name = 'DeltaPure';
    config.kernel.n_conn_kernel = 3;
    config.kernel.cortex_scale = [1.0, 0.6, 0.3];
    config.kernel.tc_scale = [1.0, 0.6, 0.3];
    config.reference.epoch = 3000;
    config.reference.fold_idx = 0;
    config.reference.shuffle_idx = 0;
    ...
    % Simulated conditions. Preallocate from a fully defined template.
    % Do not use struct([]) followed by whole-structure indexed assignment:
    % an empty no-field struct is dissimilar to a populated condition struct.
    condition_template = struct( ...
        ...
        'sync_on_offset', 1.5, ...
        'sync_off_offset', -0.6);
    conditions = repmat(condition_template, 1, 3);
...
function config = apply_model_size_mode(config)
    switch config.model_size_source
        ...
        case 'fixed'
            % Fixed mode is independent of reference raster and GLM data.
            % The externally selected temporal-kernel file is still loaded.
...
function output_manifest = save_simulation_output( ...
    ...
    output_manifest.raster_paths = strings(n_conditions, n_areas);
    output_manifest.border_paths = strings(n_conditions, n_areas);
    ...
    for condition_i = 1:n_conditions
        ...
        for area_i = 1:n_areas
            ...
            raster_meta = build_output_raster_meta( ...
                config, sim_meta, result_view, condition_i, area_label);
            ...
            if config.output.save_project_raster
                ...
                progress_log('SAVE-RASTER', ...
                    'Writing %s view: %s', area_label, raster_path);
                save(raster_path, 'meta', 'data', '-v7.3');
                output_manifest.raster_paths(condition_i, area_i) = ...
                    string(raster_path);
            end
            ...
            if config.output.save_project_border
                ...
                progress_log('SAVE-BORDER', ...
                    'Writing %s view: %s', area_label, border_path);
                save(border_path, 'meta', 'data');
                output_manifest.border_paths(condition_i, area_i) = ...
                    string(border_path);
            end
        end
    end
...
function meta = build_output_raster_meta( ...
    config, sim_meta, result, condition_i, area_label)
