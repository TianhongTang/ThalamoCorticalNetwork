...
plot_type = 'either_significant';

x_range = [-3, 3];
y_range = [-0.2, 0.2];

alt_x_prepost = 'Pre';
alt_x_state = 'RestClose';
alt_x_kernel_idx = 1;
alt_x_axis_label = sprintf('%s %s K%d', alt_x_prepost, alt_x_state, alt_x_kernel_idx);

alt_y_prepost = 'Pre';
alt_y_state = 'RestClose';
alt_y_kernel_idx = 2;
alt_y_axis_label = sprintf('%s %s K%d', alt_y_prepost, alt_y_state, alt_y_kernel_idx);

alt_plot_type = 'all_connection';
...
key_alt_x = state_key(alt_x_prepost, alt_x_state, alt_x_kernel_idx);
key_alt_y = state_key(alt_y_prepost, alt_y_state, alt_y_kernel_idx);

pooled_data = empty_connection_data();
pooled_data_alt = empty_connection_data();
...
for session_i = 1:numel(loaded_session_data)
    ...
    if ~isfield(loaded_states, key_alt_x)
        error('alt x condition was not loaded: %s. Add it to data_preposts/data_states/data_kernel_indices and rerun Load data.', key_alt_x);
    end
    if ~isfield(loaded_states, key_alt_y)
        error('alt y condition was not loaded: %s. Add it to data_preposts/data_states/data_kernel_indices and rerun Load data.', key_alt_y);
    end

    state_x = loaded_states.(key_x);
    state_y = loaded_states.(key_y);
    state_alt_x = loaded_states.(key_alt_x);
    state_alt_y = loaded_states.(key_alt_y);
    ...
    pair_data_alt = make_pair_vectors(state_alt_x, state_alt_y, err_multi, meta, session_label);
    pooled_data_alt = append_connection_data(pooled_data_alt, pair_data_alt);
end
...
plot_data_alt = prepare_plot_data(pooled_data_alt, alt_plot_type, alt_x_axis_label, alt_y_axis_label);

selected_uid = make_connection_uid(selected_data);
plot_data_alt_uid = make_connection_uid(plot_data_alt);
highlight_mask_alt = ismember(plot_data_alt_uid, selected_uid);
n_selected_primary = numel(selected_uid);
n_highlight_alt = sum(highlight_mask_alt);

fprintf('\nPrimary selected connections: %d\n', n_selected_primary);
fprintf('Selected connections that appear in alternate plot: %d\n', n_highlight_alt);

% Plot full scatter, zoomed selection, and alternate plot
...
ax = nexttile(3);
plot_scatter_groups(ax, plot_data_alt, scatter_marker_size, scatter_alpha, colors, show_legend);
overlay_highlighted_points(ax, plot_data_alt, highlight_mask_alt, scatter_marker_size + 18, colors.region_box);
axis_limit_alt = get_axis_limit_for_plot(plot_data_alt, alt_plot_type);
xlim(ax, axis_limit_alt.x);
ylim(ax, axis_limit_alt.y);
axis(ax, 'square');
xlabel(ax, plot_data_alt.x_label);
ylabel(ax, plot_data_alt.y_label);
title(ax, sprintf('Alternate plot\nmatched from panel 1: %d / %d', n_highlight_alt, n_selected_primary), 'Interpreter', 'none');

% Optional text output

if text_out
    print_mapped_connections(selected_data, plot_data_alt);
end
...
if save_text_out
    ...
    text_path = fullfile(save_folder, sprintf('Figure3_region_selection_mapping_%s_to_%s.txt', ...
        sanitize_filename(plot_type), sanitize_filename(alt_plot_type)));
    write_mapped_connections_to_file(selected_data, plot_data_alt, text_path);
    fprintf('Saved mapped selected-connection text to:\n%s\n', text_path);
end
...
function data = make_pair_vectors(state_x, state_y, err_multi, meta, session_label)
    ...
    [to12_global, from12_global, to12_area, from12_area] = make_connection_index_vectors(state_x.idx1, state_x.idx2);
    [to21_global, from21_global, to21_area, from21_area] = make_connection_index_vectors(state_x.idx2, state_x.idx1);
    ...
    n12 = numel(x12);
    n21 = numel(x21);
    from_area = [repmat("VLPFC", n12, 1); repmat("ACC", n21, 1)];
    to_area = [repmat("ACC", n12, 1); repmat("VLPFC", n21, 1)];
    connection_direction = [repmat("VLPFC_to_ACC", n12, 1); repmat("ACC_to_VLPFC", n21, 1)];
...
function print_mapped_connections(selected_data, plot_data_alt)
    [selected_order, alt_idx_for_selected] = get_mapping_order(selected_data, plot_data_alt);
    n_match = sum(alt_idx_for_selected > 0);

    fprintf('\n===== Mapped connections: %d / %d; sorted by primary plot_y descending =====\n', ...
        n_match, numel(selected_order));

    for rank_i = 1:numel(selected_order)
        i = selected_order(rank_i);
        alt_i = alt_idx_for_selected(rank_i);

        fprintf('%03d | plot_x=%.6g, plot_y=%.6g, raw_x=%.6g, raw_y=%.6g, session="%s", %s(area_idx=%d, global_idx=%d) -> %s(area_idx=%d, global_idx=%d), direction=%s, group=%s\n', ...
            rank_i, ...
        ...
        if alt_i > 0
            fprintf('    J(%d,%d): (%.6g, %.6g) -> (%.6g, %.6g)\n\n', ...
                selected_data.to_cell_global_idx(i), selected_data.from_cell_global_idx(i), ...
                selected_data.x_plot(i), selected_data.y_plot(i), ...
                plot_data_alt.x_plot(alt_i), plot_data_alt.y_plot(alt_i));
        else
            fprintf('    J(%d,%d): (%.6g, %.6g) -> not found in alternate plot\n\n', ...
                selected_data.to_cell_global_idx(i), selected_data.from_cell_global_idx(i), ...
                selected_data.x_plot(i), selected_data.y_plot(i));
        end
    end
...
function write_mapped_connections_to_file(selected_data, plot_data_alt, text_path)
    fid = fopen(text_path, 'w');
    cleaner = onCleanup(@() fclose(fid)); %#ok<NASGU>

    [selected_order, alt_idx_for_selected] = get_mapping_order(selected_data, plot_data_alt);

    fprintf(fid, 'rank\tplot_x\tplot_y\traw_x\traw_y\tsession\tfrom_area\tfrom_cell_area_idx\tfrom_cell_global_idx\tto_area\tto_cell_area_idx\tto_cell_global_idx\tdirection\tgroup\n');
    fprintf(fid, 'Jij\tprimary_x\tprimary_y\talternate_x\talternate_y\n\n');

    for rank_i = 1:numel(selected_order)
        i = selected_order(rank_i);
        alt_i = alt_idx_for_selected(rank_i);

        fprintf(fid, '%d\t%.12g\t%.12g\t%.12g\t%.12g\t%s\t%s\t%d\t%d\t%s\t%d\t%d\t%s\t%s\n', ...
            rank_i, ...
            selected_data.x_plot(i), selected_data.y_plot(i), ...
            selected_data.x_raw(i), selected_data.y_raw(i), ...
            selected_data.session_label(i), ...
        ...
        if alt_i > 0
            fprintf(fid, 'J(%d,%d)\t%.12g\t%.12g\t%.12g\t%.12g\n\n', ...
                selected_data.to_cell_global_idx(i), selected_data.from_cell_global_idx(i), ...
                selected_data.x_plot(i), selected_data.y_plot(i), ...
                plot_data_alt.x_plot(alt_i), plot_data_alt.y_plot(alt_i));
        else
            fprintf(fid, 'J(%d,%d)\t%.12g\t%.12g\tNA\tNA\n\n', ...
                selected_data.to_cell_global_idx(i), selected_data.from_cell_global_idx(i), ...
                selected_data.x_plot(i), selected_data.y_plot(i));
        end
    end
...
function [selected_order, alt_idx_for_selected] = get_mapping_order(selected_data, plot_data_alt)
    [~, selected_order] = sort(selected_data.y_plot, 'descend', 'MissingPlacement', 'last');

    selected_uid = make_connection_uid(selected_data);
    alt_uid = make_connection_uid(plot_data_alt);

    alt_idx_for_selected = zeros(numel(selected_order), 1);

    for rank_i = 1:numel(selected_order)
        i = selected_order(rank_i);
        match_idx = find(alt_uid == selected_uid(i), 1, 'first');
        if ~isempty(match_idx)
            alt_idx_for_selected(rank_i) = match_idx;
        end
    end
end


function uid = make_connection_uid(data)
    uid = string(data.session_label) + "|" + ...
          string(data.from_cell_global_idx) + "|" + ...
          string(data.to_cell_global_idx);
end

function overlay_highlighted_points(ax, plot_data, highlight_mask, marker_size, marker_color)
    hold(ax, 'on');
    if any(highlight_mask)
        scatter(ax, plot_data.x_plot(highlight_mask), plot_data.y_plot(highlight_mask), marker_size, ...
            'o', 'LineWidth', 1.5, 'MarkerEdgeColor', marker_color, 'MarkerFaceColor', 'none', ...
            'DisplayName', 'matched from panel 1');
    end
    hold(ax, 'off');
end
