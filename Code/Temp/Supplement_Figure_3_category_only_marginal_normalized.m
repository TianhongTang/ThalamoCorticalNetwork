...
%   Columns = raw count, row-normalized, column-normalized, total-normalized, marginal_normalized.
...
function render_comparison_figure(root, cfg, pooled_one, params)
    ...
    for ctx_i = 1:numel(cfg.contexts)
        ...
        counts_marginal_norm = normalize_transition_matrix(counts, 'marginal');
        ...
        counts_2x2_marginal_norm = normalize_transition_matrix(counts_2x2, 'marginal');
        ...
        % Column 5: marginal_normalized observed/chance ratio
        ax = nexttile(tile_idx(row_top, 5));
        plot_transition_table_generic(ax, counts_marginal_norm, params.category_labels, agreement, kappa, n_cat, ...
            sprintf('%s categorical 3x3', ctx.name), sprintf('%s category', ctx.x.axis_label), sprintf('%s category', ctx.y.axis_label), ...
            'marginal_normalized', 'observed / chance', 'fraction');
        add_panel_label(ax, row_top, 5, params.n_col);

        ax = nexttile(tile_idx(row_bottom, 5));
        plot_transition_table_generic(ax, counts_2x2_marginal_norm, category_labels_2x2, agreement_2x2, kappa_2x2, n_cat_2x2, ...
            sprintf('%s categorical 2x2', ctx.name), sprintf('%s category', ctx.x.axis_label), sprintf('%s category', ctx.y.axis_label), ...
            'marginal_normalized', 'observed / chance', 'fraction');
    ...
    figWidth = 20.0;  % inches. Wider layout for 5 columns.
    ...
    switch lower(mode)
        ...
        case 'marginal'
            ...
            if total_n > 0
                ...
                chance_baseline = x_marginal_prob * y_marginal_prob;
                valid = chance_baseline > 0;
                mat_norm(valid) = observed_prob(valid) ./ chance_baseline(valid);
            end
        otherwise
