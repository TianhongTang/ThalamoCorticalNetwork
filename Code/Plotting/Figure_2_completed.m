...
%% parameters
shuffle_N = 5;       % Increase for final statistical bands, e.g. 100.
err_multi = 2;       % threshold for significant J, in multiples of GLM error estimate.
t_range = 1:60000;
corr_range = 500;   % ms, assuming 1 kHz sampling.
smooth_window = 5;  % ms.
sample_rate = 1000; % Hz.
...
for i = 1:n_state
    ...
    % Row 4: spectrum of the plotted population cross-correlogram.
    spec_Xcorr = myFT(smooth_correlogram, sample_rate, freqs);

    row = 4;
    tile = nexttile(i + n_state*(row-1));
    plot(tile, freqs, spec_Xcorr, 'm-', 'LineWidth', 1, ...
    ...
    spec_acc = myFT(pop_acc, sample_rate, freqs);
    spec_vlpfc = myFT(pop_vlpfc, sample_rate, freqs);
    spec_AutoAcc = myFT(smooth_auto_acc, sample_rate, freqs);
    spec_AutoVlpfc = myFT(smooth_auto_vlpfc, sample_rate, freqs);

    row = 5;
    tile = nexttile(i + n_state*(row-1));
    plot(tile, freqs, spec_acc, 'r-', 'LineWidth', 1, ...
        'DisplayName', 'ACC population signal');
    hold(tile, 'on');
    plot(tile, freqs, spec_vlpfc, 'b-', 'LineWidth', 1, ...
        'DisplayName', 'VLPFC population signal');
    plot(tile, freqs, spec_AutoAcc, 'r--', 'LineWidth', 1, ...
        'DisplayName', 'ACC auto-corr');
    plot(tile, freqs, spec_AutoVlpfc, 'b--', 'LineWidth', 1, ...
