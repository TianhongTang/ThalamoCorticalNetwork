% Further analysis of population cross-correlogram across all sessions.
...
%% Main
animal_names = {'Slayer', 'Slayer'};
injections = {'Muscimol', 'Saline'};
session_nums = [8, 5];
condition_num = length(animal_names);
corr_range = 1000;
smooth_window = 25;
for ci = 1:condition_num
    for session_idx = 1:session_nums(ci)
        
        meta = struct();
        meta.animal_name = animal_names{ci};
        meta.injection = injections{ci};
        meta.align = 'Last';
        ...
        % filename = fullfile(save_folder, sprintf('Figure1_%s_%s_%d.pdf', meta.animal_name, meta.injection, meta.session_idx));
        ...
        filename = fullfile(save_folder, sprintf('Figure1_%s_%s_%d_preview.jpg', meta.animal_name, meta.injection, meta.session_idx));
        exportgraphics(fig, filename, ...
            'ContentType', 'image', ...
            'BackgroundColor', 'white', ...
            'Resolution', resolution);
