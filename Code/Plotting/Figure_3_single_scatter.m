...
show_fit_line = true;
fit_line_method = 'tls'; % 'ols' minimizes y residuals; 'tls' minimizes orthogonal distances.
...
%   'same_abs'
%   'switch_abs'
single_scatter_plot_mode = 'both_sig_signed';
...
for row_i = 1:n_panel_row
    for col_i = 1:n_panel_col
        ...
        plot_pair_scatter(ax, cfg.data, single_scatter_plot_mode, scatter_marker_size, scatter_alpha, ...
            ...
            axis_limit_override, show_inset_stat, show_identity_line, show_fit_line, fit_line_method);
...
function [x_fit, y_fit] = total_least_squares_line(x, y, axis_limit)
    ...
    xlim_vals = axis_limit;
    ylim_vals = axis_limit;

    candidates = [];

    if abs(direction(1)) > eps
        t = (xlim_vals(1) - mu(1)) / direction(1);
        y_candidate = mu(2) + t * direction(2);
        if y_candidate >= ylim_vals(1) && y_candidate <= ylim_vals(2)
            candidates = [candidates; xlim_vals(1), y_candidate]; %#ok<AGROW>
        end

        t = (xlim_vals(2) - mu(1)) / direction(1);
        y_candidate = mu(2) + t * direction(2);
        if y_candidate >= ylim_vals(1) && y_candidate <= ylim_vals(2)
            candidates = [candidates; xlim_vals(2), y_candidate]; %#ok<AGROW>
        end
    end

    if abs(direction(2)) > eps
        t = (ylim_vals(1) - mu(2)) / direction(2);
        x_candidate = mu(1) + t * direction(1);
        if x_candidate >= xlim_vals(1) && x_candidate <= xlim_vals(2)
            candidates = [candidates; x_candidate, ylim_vals(1)]; %#ok<AGROW>
        end

        t = (ylim_vals(2) - mu(2)) / direction(2);
        x_candidate = mu(1) + t * direction(1);
        if x_candidate >= xlim_vals(1) && x_candidate <= xlim_vals(2)
            candidates = [candidates; x_candidate, ylim_vals(2)]; %#ok<AGROW>
        end
    end
    ...
    % Choose the two farthest intersection points within the visible square.
