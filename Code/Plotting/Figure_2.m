%% Figure 2: Session-flexible population-level ACC-VLPFC analyses.
% Same layout and spectral-control logic as Figure 1, replacing manually
% selected neuron pairs with ACC/VLPFC population-average firing-rate traces.
%
% Main change from the older Figure 2:
%   - analysis uses all trials by default, not one hard-coded trial/time window.
%   - analysis_t_range = [] uses the full available length of each trial.
%   - display_t_range only controls Column 1 visualization.
%   - change the meta fields below to generate a different session.
...
%% Figure layout
% Column 1 = ACC/VLPFC population firing-rate traces.
% Column 2 = population cross-correlogram.
% Column 3 = population auto-correlograms.
% Column 4 = population cross-PSD.
% Column 5 = ACC/VLPFC population PSDs.
% Column 6 = ACC-VLPFC population spectral coherence.
n_column = 6;
figure_visible = 'off';
...
for i = 1:n_state
    ...
    [pop_acc_trials, pop_vlpfc_trials] = get_population_trial_segments( ...
        rasters, filter_acc, filter_vlpfc, ...
        analysis_trial_mode, analysis_trial_idx, analysis_t_range, corr_range);

    [display_time_ms, pop_acc_display_plot, pop_vlpfc_display_plot] = get_population_display_trace( ...
        rasters, filter_acc, filter_vlpfc, display_trial_idx, display_t_range, ...
        sample_rate, smooth_kernel);

    fprintf('Population firing rates for %s %s %s:\n', meta.prepost, meta.state, meta.area);
    fprintf('ACC population mean: %.3f Hz/neuron, N=%d, trials=%d\n', ...
        weighted_signal_mean(pop_acc_trials, sample_rate), N_acc, numel(pop_acc_trials));
    fprintf('VLPFC population mean: %.3f Hz/neuron, N=%d, trials=%d\n', ...
        weighted_signal_mean(pop_vlpfc_trials, sample_rate), N_vlpfc, numel(pop_vlpfc_trials));

    %% Column 1: population firing-rate traces.
    column = 1;
    ...
    [correlogram, lags] = aggregate_norm_xcorr_trials(pop_acc_trials, pop_vlpfc_trials, corr_range, lag_weight_correction);
    [auto_acc, ~] = aggregate_norm_xcorr_trials(pop_acc_trials, pop_acc_trials, corr_range, lag_weight_correction);
    [auto_vlpfc, ~] = aggregate_norm_xcorr_trials(pop_vlpfc_trials, pop_vlpfc_trials, corr_range, lag_weight_correction);
    ...
    [psd_acc, psd_vlpfc, cpsd, coherence, spec_freqs] = aggregate_pair_spectra_trials( ...
        pop_acc_trials, pop_vlpfc_trials, sample_rate, freqs, psd_window_sec, psd_overlap_frac);
    ...
    for j = 1:shuffle_N
        ...
        [shuffled_corr, shuffled_auto_acc, shuffled_auto_vlpfc, ...
            shuffled_psd_acc, shuffled_psd_vlpfc, shuffled_cpsd, shuffled_coh_j] = ...
            aggregate_control_metrics_trials(pop_acc_trials, pop_vlpfc_trials, corr_range, lag_weight_correction, ...
                sample_rate, freqs, psd_window_sec, psd_overlap_frac, 'shuffled');

        shuffle_correlograms(j, :) = same_conv(shuffled_corr, smooth_kernel);
        shuffle_auto_acc_controls(j, :) = same_conv(shuffled_auto_acc, smooth_kernel);
        shuffle_auto_vlpfc_controls(j, :) = same_conv(shuffled_auto_vlpfc, smooth_kernel);
        ...
        [shifted_corr, shifted_auto_acc, shifted_auto_vlpfc, ...
    ...
    shuffle_std = std_omitnan(shuffle_correlograms); %#ok<NASGU>
    ...
    %% Column 3: population auto-correlograms with shifted self-controls.
    column = 3;
    tile = nexttile(column + n_column*(i-1));
    col_axes(i, column) = tile;
    ...
    %% Column 4: population cross-PSD with shifted control.
    column = 4;
    ...
    %% Column 5: population PSDs with shuffled controls.
    column = 5;
    tile = nexttile(column + n_column*(i-1));
    col_axes(i, column) = tile;
    ...
    %% Column 6: population spectral coherence.
    column = 6;
    selected_control = 'shifted'; % 'shuffled' or 'shifted'
    if strcmp(selected_control, 'shuffled')
...
output_stub = sprintf('Figure2_population_alltrials_%s_%s_session%s', ...
    sanitize_filename(meta.animal_name), sanitize_filename(meta.injection), ...
    sanitize_filename(char(string(meta.session_idx))));

sgtitle(sprintf('Population ACC-VLPFC | %s %s session %s | %s trials', ...
