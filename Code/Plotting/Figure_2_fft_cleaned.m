...
t_range = 1:60000;
corr_range = 1000;   % ms, assuming 1 kHz sampling.
smooth_window = 5;  % ms.
spec_smooth_window = 5; % frequency-bin smoothing for FFT visualization only.
sample_rate = 1000; % Hz.
...
for i = 1:n_state
    ...
    % Row 5: spectra of population signals and population auto-correlograms.
    spec_acc = myFT(remove_dc(pop_acc), sample_rate, freqs);
    spec_vlpfc = myFT(remove_dc(pop_vlpfc), sample_rate, freqs);
    spec_AutoAcc = myFT(remove_dc(auto_acc), sample_rate, freqs);
    spec_AutoVlpfc = myFT(remove_dc(auto_vlpfc), sample_rate, freqs);

    spec_acc_plot = smooth_spectrum_for_plot(spec_acc, spec_smooth_window);
    spec_vlpfc_plot = smooth_spectrum_for_plot(spec_vlpfc, spec_smooth_window);
    spec_AutoAcc_plot = smooth_spectrum_for_plot(spec_AutoAcc, spec_smooth_window);
    spec_AutoVlpfc_plot = smooth_spectrum_for_plot(spec_AutoVlpfc, spec_smooth_window);

    row = 5;
    tile = nexttile(i + n_state*(row-1));
    plot(tile, freqs, spec_acc_plot, 'r-', 'LineWidth', 1, ...
        'DisplayName', 'ACC population signal');
    hold(tile, 'on');
    plot(tile, freqs, spec_vlpfc_plot, 'b-', 'LineWidth', 1, ...
        'DisplayName', 'VLPFC population signal');
    plot(tile, freqs, spec_AutoAcc_plot, 'r-', 'LineWidth', 1, ...
        'DisplayName', 'ACC auto-corr');
    plot(tile, freqs, spec_AutoVlpfc_plot, 'b-', 'LineWidth', 1, ...
        'DisplayName', 'VLPFC auto-corr');
    hold(tile, 'off');

    title(tile, sprintf('FFT of population signals/autocorr: %s, %s', meta.prepost, meta.state));
    xlabel(tile, 'Frequency (Hz)');
    ylabel(tile, 'Amplitude');
    legend(tile, 'show', 'Location', 'northeast');
end
