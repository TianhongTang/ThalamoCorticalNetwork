...
corr_range = 500; % ms
smooth_window = 5; % ms
spec_smooth_window = 3; % frequency-bin smoothing for FFT visualization only
...
for i = 1:length(areas)
    ...
    spec_r1 = myFT(remove_dc(r1), sample_rate, freqs);
    spec_r2 = myFT(remove_dc(r2), sample_rate, freqs);
    spec_Xcorr = myFT(remove_dc(correlogram), sample_rate, freqs);
    spec_Auto1 = myFT(remove_dc(auto1), sample_rate, freqs);
    spec_Auto2 = myFT(remove_dc(auto2), sample_rate, freqs);

    spec_r1_plot = smooth_spectrum_for_plot(spec_r1, spec_smooth_window);
    spec_r2_plot = smooth_spectrum_for_plot(spec_r2, spec_smooth_window);
    spec_Xcorr_plot = smooth_spectrum_for_plot(spec_Xcorr, spec_smooth_window);
    spec_Auto1_plot = smooth_spectrum_for_plot(spec_Auto1, spec_smooth_window);
    spec_Auto2_plot = smooth_spectrum_for_plot(spec_Auto2, spec_smooth_window);

    % xcorr spectrum
    row = 4;
    nexttile(i+n_state*(row-1));
    plot(freqs, spec_Xcorr_plot, 'm-', 'LineWidth', 1, 'DisplayName', 'Cross-corr');

    % signal spectrum and auto-corr spectrum
    row = 5;
    nexttile(i+n_state*(row-1));
    plot(freqs, spec_r1_plot, 'r-', 'LineWidth', 1, 'DisplayName', 'Signal 1');
    hold on;
    plot(freqs, spec_r2_plot, 'b-', 'LineWidth', 1, 'DisplayName', 'Signal 2');
    plot(freqs, spec_Auto1_plot, 'r--', 'LineWidth', 1, 'DisplayName', 'Auto-corr 1');
    plot(freqs, spec_Auto2_plot, 'b--', 'LineWidth', 1, 'DisplayName', 'Auto-corr 2');
    hold off;
