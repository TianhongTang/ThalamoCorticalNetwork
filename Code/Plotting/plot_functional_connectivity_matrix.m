            ...
            for align_idx = 1:length(alignments)
                ...
                for prepost_idx = 1:length(prepost_types)
                    ...
                    for state_idx = 1:length(states)
                        ...
                        prepost_states{end+1} = sprintf('%s%s', prepost, state);
                    end
                end
...
for task_idx = 1:length(tasks)
    task = tasks{task_idx};
    for prepost_state_idx = 1:length(task.prepost_states)
        prepost_state = task.prepost_states{prepost_state_idx};
        task_name = sprintf('%s%s%sAlign%s', task.control, prepost_state, task.area_type, task.alignment);

        % load model data
        folder_name = fullfile(root, 'Data', 'Working', 'GLM_models');
        file_name = sprintf('GLM_%s_s%d_shuffle%d_%s_%s_epoch%d_fold0.mat', task_name, task.session_idx, 0, task.kernel, ...
        ...
        for kernel_idx = 1:n_conn_kernel
            ...
            % weight matrix plot
            nexttile(prepost_state_idx + (kernel_idx-1)*length(task.prepost_states));
            hold on;
            imagesc(J_mat);
            ...
            title(sprintf('%s, Kernel %d', prepost_state, kernel_idx));
            xlabel('From Neuron');
            ylabel('To Neuron');
        end
    end
    ...
    figure_name = sprintf('FC_%s_%d.png', task.control, task.session_idx);
    figure_path = fullfile(folder_name, figure_name);
    saveas(f, figure_path);
    set(f, 'PaperUnits', 'inches');
    set(f, 'PaperPosition', [0 0 16 8]);
    set(f, 'PaperSize', [16 8]);
    pdf_name = sprintf('FC_%s_%d.pdf', task.control, task.session_idx);
